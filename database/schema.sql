...
-- Dashboard Layout Manager: atomic server-only bulk reorder for public cards.
...
as $$
begin
  if items is null or jsonb_typeof(items) <> 'array' then raise exception 'INVALID_TOOL_ORDER'; end if;
  if jsonb_array_length(items) < 1 or jsonb_array_length(items) > 200 then raise exception 'INVALID_TOOL_ORDER_SIZE'; end if;
  if exists (select 1 from jsonb_to_recordset(items) as x(id text, sort_order integer) where x.id is null or x.id !~ '^[a-z0-9][a-z0-9_-]{1,79}$' or x.sort_order is null or x.sort_order < -10000 or x.sort_order > 10000) then raise exception 'INVALID_TOOL_ORDER_ITEM'; end if;
  if exists (select x.id from jsonb_to_recordset(items) as x(id text, sort_order integer) group by x.id having count(*) > 1) then raise exception 'DUPLICATE_TOOL_ORDER_ID'; end if;
  if exists (select 1 from jsonb_to_recordset(items) as x(id text, sort_order integer) left join public.tools t on t.id = x.id where t.id is null) then raise exception 'UNKNOWN_TOOL_ORDER_ID'; end if;
  update public.tools as t set sort_order = x.sort_order, updated_at = now() from jsonb_to_recordset(items) as x(id text, sort_order integer) where t.id = x.id;
  return query select t.id, t.sort_order from public.tools t join jsonb_to_recordset(items) as x(id text, sort_order integer) on x.id = t.id order by t.sort_order asc, t.name asc;
end;
$$;
...
values
  ...
  ('animegallery', 'Nexora Anime Discover', 'Visual discovery anime, artist, source, dan reaction GIF dari Waifu.im + NEKOSBEST', 'tools', 'NEW · SFW', 'fa-regular fa-images', null, true, 19, '{"providers":["waifu-im","nekosbest"],"apiKeyRequired":false,"sfwDefault":true,"serverProxy":["nekosbest"],"route":"/anime-gallery","storage":"local-only"}'::jsonb),
  ('animetoreal', 'Anime to Real', 'Ubah ilustrasi anime dari URL menjadi gambar realistis', 'tools', 'AI TRANSFORM', 'fa-solid fa-wand-magic-sparkles', null, true, 19, '{"provider":"kuroneko","serverProxy":true,"apiKeyRequired":true,"cache":"no-store","databaseRequired":false}'::jsonb),
  ('promptgenerate', 'Nexora Prompt Generator', 'Analisis gambar dengan Nexora Vision AI menjadi prompt produksi profesional', 'tools', 'VISION AI', 'fa-solid fa-wand-magic-sparkles', null, true, 20, '{"provider":"gemini","privacy":"not-stored","targets":["midjourney","flux","stable-diffusion","ideogram","video"]}'::jsonb),
  ('novelcover', 'Nexora Novel Cover Generator', 'Generator artwork cover novel multi-provider dengan Nexora Composer', 'tools', 'COVER AI', 'fa-solid fa-book-atlas', null, true, 25, '{"providers":["ideogram","recraft","fal","runware","stability","openai","huggingface"],"privacy":"not-stored","defaultMode":"auto"}'::jsonb),
  ('elevenlabs', 'Nexora ElevenLabs Studio', 'Text to Speech, Voice Changer, Speech to Text dan Sound FX', 'tools', 'AUDIO AI', 'fa-solid fa-wave-square', null, true, 27, '{"provider":"elevenlabs","serverProxy":true,"serverKey":true,"privacy":"not-stored","features":["tts","voice-changer","speech-to-text","sound-effects"]}'::jsonb),
  ('fakeovo', 'Fake OVO', 'Simulasi OVO dengan penanda simulasi minimal', 'tools', 'SIMULASI', 'fa-solid fa-wallet', null, true, 30, '{}'::jsonb),
  ...
  ('documentai', 'Nexora Document AI', 'Ringkas, analisis, ekstrak tabel, dan tanya isi dokumen dengan Nexora AI', 'tools', 'VVIP', 'fa-solid fa-file-waveform', null, true, 69, '{"provider":"gemini","serverKey":true,"privacy":"not-stored","features":["summary","deep-analysis","study-notes","table-extraction","document-qa","exports"]}'::jsonb),
  ('autopdf', 'Nexora Auto PDF', 'Ubah teks panjang menjadi PDF A5 rapi dengan pagination otomatis', 'tools', 'PDF', 'fa-solid fa-file-pdf', null, true, 69, '{"provider":"nexora-pdf-api","serverProxy":true,"serverKey":true,"privacy":"not-stored","maxSections":5,"features":["a5","a4","automatic-pagination","preview","download"]}'::jsonb),

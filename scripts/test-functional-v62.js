...
assert.equal(registry.list().length, 40);
assert.equal(new Set(registry.list().map((item) => item.id)).size, 40);
...
assert.equal(healthCatalog.length, 40);
...
assert.equal(new Set(seedIds).size, 40, "Seed database harus memuat seluruh 40 tools");
...
console.log("Nexora v6.2 tests lulus: 40-tool registry, functional audit, stable lazy dispatch, health catalog lengkap, mobile nav 5 menu, dan Vercel Hobby valid.");

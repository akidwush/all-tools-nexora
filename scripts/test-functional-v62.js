...
assert.equal(registry.count, 39);
assert.equal(registry.list().length, 39);
assert.equal(new Set(registry.list().map((item) => item.id)).size, 39);
...
assert.equal(healthCatalog.length, 39);
...
assert.equal(new Set(seedIds).size, 39, "Seed database harus memuat seluruh 39 tools");
...
console.log("Nexora v6.2 tests lulus: 39-tool registry, functional audit, stable lazy dispatch, health catalog lengkap, mobile nav 5 menu, dan Vercel Hobby valid.");

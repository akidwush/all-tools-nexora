...
assert.equal(registry.count, EXPECTED_TOOL_COUNT);
assert.equal(registry.list().length, EXPECTED_TOOL_COUNT);
assert.equal(new Set(registry.list().map((item) => item.id)).size, EXPECTED_TOOL_COUNT);
...
assert.equal(healthCatalog.length, EXPECTED_TOOL_COUNT);
...
assert.equal(new Set(seedIds).size, EXPECTED_TOOL_COUNT - 4, "Seed database hanya memuat tool yang memang database-backed");
...
console.log(`Regression katalog lulus: ${EXPECTED_TOOL_COUNT} tool, health/seed sinkron, lazy dispatch stabil, mobile nav valid.`);

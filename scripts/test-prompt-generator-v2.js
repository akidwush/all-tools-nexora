...
async function main() {
  ...
  try {
    ...
    for (const token of ["renderPromptGenerator", "/api/prompt-generator", "Image to Video", "negative prompt", "Gambar tidak disimpan", "createImageBitmap", "application/octet-stream", "maxSide=1600", "nexoraTimeoutMs:85000"]) assert.ok(client.includes(token));
    ...
    assert.match(read("assets/js/core/lazy-loader.js"), /prompt-v2-state4-android/);
    assert.match(read("index.html"), /prompt-v2-state4-android/);

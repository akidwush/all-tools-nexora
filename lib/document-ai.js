...
function providerError(error) {
  ...
  if (status === 429 || providerCode === "RESOURCE_EXHAUSTED" || /quota|rate limit|resource exhausted/.test(message)) return Object.assign(new Error("Layanan AI sedang sibuk atau kuotanya tercapai. Coba lagi sesaat."), { code: "DOCUMENT_AI_PROVIDER_LIMIT", status: 429 });
  if ([401, 403].includes(status) || ["UNAUTHENTICATED", "PERMISSION_DENIED"].includes(providerCode) || /api key|unauth|forbidden|permission denied/.test(message)) return Object.assign(new Error("Kredensial Nexora AI ditolak. Periksa environment di Vercel, lalu redeploy."), { code: "DOCUMENT_AI_AUTH_FAILED", status: 503 });
  if (status === 404 || providerCode === "NOT_FOUND" || /model.+(not found|not supported|not available)|unknown model/.test(message)) return Object.assign(new Error("Model Nexora AI yang dipilih tidak tersedia."), { code: "DOCUMENT_AI_MODEL_UNAVAILABLE", status: 502 });
  return Object.assign(new Error("Dokumen belum dapat dianalisis oleh Nexora AI."), { code: "DOCUMENT_AI_PROVIDER_ERROR", status: 502 });
}

async function generate({ document, prompt, json = false, timeoutMs = 55_000, clientFactory }) {
  const { apiKey } = resolveGeminiApiKey();
  if (!apiKey) throw Object.assign(new Error("Nexora AI belum dikonfigurasi. Tambahkan kredensial provider di Vercel lalu redeploy."), { code: "DOCUMENT_AI_NOT_CONFIGURED", status: 503 });
  const client = typeof clientFactory === "function"
    ? await clientFactory(apiKey)
    : new (await import("@google/genai")).GoogleGenAI({ apiKey });
  const modelCandidates = geminiModelCandidates("DOCUMENT_AI_MODEL", DEFAULT_MODEL);
  const totalTimeout = Math.max(1000, Number(timeoutMs) || 55_000);
  ...
  for (const [modelIndex, model] of modelCandidates.entries()) {
    ...
    const attemptTimeout = modelCandidates.length > 1 ? Math.min(modelIndex === 0 ? 30_000 : 18_000, remaining) : remaining;
    ...
    try {
      ...
      if (!text) {
        ...
        console.warn("[document-ai] empty model response", { model, ...diagnostics });
        const blocked = diagnostics.blockReason || diagnostics.finishReasons.some((reason) => /SAFETY|BLOCK|PROHIBITED/.test(reason));
        if (blocked) throw Object.assign(new Error("Dokumen ditolak oleh filter keamanan Nexora AI. Coba dokumen lain."), { code: "DOCUMENT_AI_BLOCKED", status: 422 });
        throw Object.assign(new Error("Nexora AI tidak menghasilkan respons."), { code: "DOCUMENT_AI_EMPTY_RESPONSE", status: 502 });
      }
      if (json) parseJsonResponse(text);
      return text;
    } catch (error) {
      lastError = providerError(error);
      if (!actionableError || lastError.code === "DOCUMENT_AI_TIMEOUT" || actionableError.code === "DOCUMENT_AI_MODEL_UNAVAILABLE") actionableError = lastError;
      const canFallback = modelIndex < modelCandidates.length - 1 && [
        ...
        "DOCUMENT_AI_PROVIDER_ERROR"
      ].includes(lastError.code);
      if (canFallback) console.warn("[document-ai] model fallback", { model, code: lastError.code, nextModel: modelCandidates[modelIndex + 1] });
...
async function handleDocumentAi(request, response) {
  ...
  if (request.method === "GET") {
    const keyConfig = resolveGeminiApiKey();
    return send(response, 200, {
      ok: true,
      configured: keyConfig.configured,
      model: geminiModelCandidates("DOCUMENT_AI_MODEL", DEFAULT_MODEL)[0],
...
module.exports = { ALLOWED_MIME_TYPES, MAX_BINARY_BYTES, generate, handleDocumentAi, parseDocument, parseJsonResponse };

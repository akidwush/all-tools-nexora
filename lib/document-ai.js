...
const { GEMINI_API_KEY_ENV_NAMES, geminiErrorDetails, geminiLowLatencyConfig, geminiModelCandidates, resolveGeminiApiKey } = require("./gemini-config");
...
async function generate({ document, prompt, json = false, timeoutMs = 55_000, clientFactory }) {
  ...
  for (const [modelIndex, model] of modelCandidates.entries()) {
    ...
    const attemptTimeout = modelCandidates.length > 1 ? Math.min(modelIndex === 0 ? 46_000 : 22_000, remaining) : remaining;
    ...
    try {
      ...
      const canFallback = modelIndex < modelCandidates.length - 1 && ["DOCUMENT_AI_TIMEOUT", "DOCUMENT_AI_MODEL_UNAVAILABLE"].includes(lastError.code);

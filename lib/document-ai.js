...
function providerError(error) {
  ...
  if ([401, 403].includes(status) || ["UNAUTHENTICATED", "PERMISSION_DENIED"].includes(providerCode) || /api key|unauth|forbidden|permission denied/.test(message)) return Object.assign(new Error("Gemini API key ditolak. Periksa key dan scope environment di Vercel, lalu redeploy."), { code: "DOCUMENT_AI_AUTH_FAILED", status: 503 });
  if (status === 404 || providerCode === "NOT_FOUND" || /model.+(not found|not supported|not available)|unknown model/.test(message)) return Object.assign(new Error("Model Gemini yang dipilih tidak tersedia."), { code: "DOCUMENT_AI_MODEL_UNAVAILABLE", status: 502 });
...
async function generate({ document, prompt, json = false, timeoutMs = 55_000, clientFactory }) {
  ...
  if (!apiKey) throw Object.assign(new Error("Gemini API key belum dikonfigurasi. Tambahkan GEMINI_API_KEY di Vercel lalu redeploy."), { code: "DOCUMENT_AI_NOT_CONFIGURED", status: 503 });
  ...
  for (const [modelIndex, model] of modelCandidates.entries()) {
    ...
    try {
      ...
      if (!text) {
        ...
        if (blocked) throw Object.assign(new Error("Dokumen ditolak oleh filter keamanan Gemini. Coba dokumen lain."), { code: "DOCUMENT_AI_BLOCKED", status: 422 });
        throw Object.assign(new Error("Gemini tidak menghasilkan respons."), { code: "DOCUMENT_AI_EMPTY_RESPONSE", status: 502 });
      }
  ...
  throw actionableError || lastError || providerError(Object.assign(new Error("Gemini timeout"), { name: "AbortError" }));
}

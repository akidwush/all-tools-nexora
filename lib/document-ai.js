...
const { GEMINI_API_KEY_ENV_NAMES, geminiErrorDetails, geminiGenerationConfig, geminiModelCandidates, resolveGeminiApiKey } = require("./gemini-config");
...
async function generate({ document, prompt, json = false, timeoutMs = 55_000, clientFactory }) {
  ...
  for (const [modelIndex, model] of modelCandidates.entries()) {
    ...
    const attemptTimeout = modelCandidates.length > 1 ? Math.min(26_000, remaining) : remaining;
    ...
    try {
      ...
      const canFallback = modelIndex < modelCandidates.length - 1 && ["DOCUMENT_AI_TIMEOUT", "DOCUMENT_AI_MODEL_UNAVAILABLE", "DOCUMENT_AI_PROVIDER_ERROR"].includes(lastError.code);

...
function multiAiEndpointDefinition(providerId, provider = PROVIDERS[providerId]) {
  ...
  return Object.freeze({
    ...
    allowedHosts: ["sylvatica.my.id"],
    defaultBaseUrl: API_ORIGIN,
    defaultPath: provider.path,
    defaultTimeoutMs: provider.timeoutMs,
    minTimeoutMs: 1_000,
    maxTimeoutMs: provider.timeoutMs > 60_000 ? 120_000 : 60_000,
    healthPath: provider.path,
    healthMethod: "HEAD",
    credentialRef: "KURONEKO_API_KEY",
    editableFields: ["baseUrl","path","timeoutMs","healthPath","fallbackProvider"]
  });
}
function multiAiEndpointDefinitions() {
  return PUBLIC_PROVIDER_IDS.map((id) => multiAiEndpointDefinition(id, PROVIDERS[id]));
}
...
async function publicRegistry() {
  ...
  const now = Date.now();
  const providers = await Promise.all(PUBLIC_PROVIDER_IDS.map(async (id) => {
    const item = health.get(id);
    const endpoint = await resolveProviderConfig(multiAiEndpointDefinition(id), { allowUnavailable: true });
    const endpointKey = endpoint.baseUrl + endpoint.path;
    const current = item && item.endpointKey === endpointKey && now - item.time <= HEALTH_TTL_MS ? item : null;
    const visible = endpoint.mode !== "disabled";
    const enabled = visible && endpoint.mode === "active" && settings.providers[id] !== false;
    const row = publicProvider(PROVIDERS[id], enabled, current);
    if (endpoint.mode === "maintenance") row.status = "maintenance";
    if (endpoint.mode === "disabled") row.status = "disabled";
    row.endpointMode = endpoint.mode;
    return row;
  }));
...
async function invokeProviderOnce(providerId, rawInput, variant, runtime = {}) {
  ...
  const endpoint = await resolveProviderConfig(multiAiEndpointDefinition(providerId, provider));
  ...
  const target = new URL(endpoint.path, endpoint.baseUrl);
  for (const [key, value] of Object.entries(input)) target.searchParams.set(key, value);
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), Math.min(endpoint.timeoutMs, Number(runtime.timeoutMs) || endpoint.timeoutMs));
  ...
  try {
    ...
    health.set(providerId, { status: "online", latencyMs, checkedAt: new Date().toISOString(), time: Date.now(), failures: 0, endpointKey: endpoint.baseUrl + endpoint.path });
    ...
    health.set(providerId, { status: failures >= 3 ? "offline" : "degraded", latencyMs: Date.now() - started, checkedAt: new Date().toISOString(), time: Date.now(), failures, endpointKey: endpoint.baseUrl + endpoint.path });
    throw normalized;
  } finally { clearTimeout(timer); }
}

function adaptFallbackInput(source, target, rawInput) {
  const raw = isObject(rawInput) ? rawInput : {};
  const output = {};
  const commonPrompt = clean(raw.prompt || raw.q || raw.query || raw.text, 8000);
  for (const field of target.fields) {
    if (Object.prototype.hasOwnProperty.call(raw, field.name)) {
      output[field.name] = raw[field.name];
      continue;
    }
    if (field.required && commonPrompt && (field.type === "text" || field.type === "textarea")) output[field.name] = commonPrompt;
    else if (field.defaultValue != null) output[field.name] = field.defaultValue;
  }
  return output;
}

async function invokeProvider(providerId, rawInput, variant, runtime = {}) {
  const visited = new Set(Array.isArray(runtime.endpointFallbackVisited) ? runtime.endpointFallbackVisited : []);
  if (visited.has(providerId)) throw Object.assign(new Error("FALLBACK_CYCLE"), { code: "FALLBACK_CYCLE", status: 503, publicMessage: "Fallback provider membentuk siklus." });
  visited.add(providerId);
  try {
    return await invokeProviderOnce(providerId, rawInput, variant, { ...runtime, endpointFallbackVisited: [...visited] });
  } catch (error) {
    const status = Number(error.status || 500);
    const fallbackEligible = status === 429 || status >= 500 || ["PROVIDER_DISABLED","PROVIDER_MAINTENANCE"].includes(error.code);
    if (!fallbackEligible) throw error;
    const source = PROVIDERS[providerId];
    if (!source) throw error;
    const endpoint = await resolveProviderConfig(multiAiEndpointDefinition(providerId, source), { allowUnavailable: true });
    const fallbackId = String(endpoint.fallbackProvider || "").replace(/^ai:/, "");
    const target = PROVIDERS[fallbackId];
    if (!fallbackId || !target || target.category !== source.category || visited.has(fallbackId)) throw error;
    const result = await invokeProvider(fallbackId, adaptFallbackInput(source, target, rawInput), "", { ...runtime, endpointFallbackVisited: [...visited] });
    return { ...result, fallbackFrom: providerId };
  }
}
...
module.exports = { API_ORIGIN, PROVIDERS, PUBLIC_PROVIDER_IDS, handleMultiAi, invokeProvider, multiAiEndpointDefinition, multiAiEndpointDefinitions, normalizeResponse, publicRegistry, validateInput };

...
const PROVIDERS = Object.freeze({
  terabox: {
    inputHosts: ["terabox.com", "teraboxapp.com", "1024tera.com", "dubox.com", "teraboxlink.com"],
...
function linkOf(item) {
  ...
  for (const key of ["download_url", "downloadUrl", "dlink", "fast_download", "fastlink", "fast_link", "direct_link", "directLink", "stream_url", "streamUrl", "playUrl", "play_url", "download", "video", "image", "url", "link"]) {
...
function normalizeTerabox(payload, officialUrl) {
  const candidates = [
    payload?.result?.file, payload?.result?.files, payload?.data?.file, payload?.data?.files,
    payload?.file, payload?.files, payload?.result?.list, payload?.data?.list, payload?.list,
    payload?.result, payload?.data
  ];
  let raw = candidates.find(Array.isArray) || [];
  if (!raw.length) {
    const single = payload?.result || payload?.data || payload;
    if (linkOf(single)) raw = [single];
  }
...
function normalizeInstagram(payload, officialUrl) {
  const source = [payload?.result?.media, payload?.result?.items, payload?.data?.media, payload?.data?.items, payload?.result, payload?.data].find(Array.isArray)
    || payload?.result?.data
    || payload?.result
    || payload?.data
    || [];
  const raw = Array.isArray(source) ? source : (source && typeof source === "object" ? [source] : []);
  ...
  return {
    title: cleanText(payload?.title || payload?.caption || "Instagram Media"),
    author: cleanText(payload?.username || payload?.owner),
    caption: cleanText(payload?.caption, 2_000),
...
module.exports = { PROVIDERS, RESPONSE_LIMIT_BYTES, fetchProviderJson, handleDownloader, hostMatches, normalizeProvider, validateProviderUrl };

...
const { handleResolvedMedia } = require("./media-download");
...
const PROVIDERS = Object.freeze({
  ...
  youtube: {
    ...
    sources: [
      { id: "siputzx-youtube-mp4", hosts: ["api.siputzx.my.id"], timeoutMs: 12_000, mediaType: "MP4", endpoint: (url) => `https://api.siputzx.my.id/api/d/ytmp4?url=${encodeURIComponent(url)}` },
      { id: "siputzx-youtube-mp3", hosts: ["api.siputzx.my.id"], timeoutMs: 12_000, mediaType: "MP3", endpoint: (url) => `https://api.siputzx.my.id/api/d/ytmp3?url=${encodeURIComponent(url)}` },
      { id: "youtube-oembed", hosts: ["youtube.com"], timeoutMs: 8_000, metadataOnly: true, endpoint: (url) => `https://www.youtube.com/oembed?format=json&url=${encodeURIComponent(url)}` }
    ],
...
function youtubeDirectUrl(payload) {
  const candidates = [
    payload?.data?.dl, payload?.data?.download, payload?.data?.download_url, payload?.data?.url,
    payload?.result?.dl, payload?.result?.download, payload?.result?.download_url, payload?.result?.url,
    payload?.dl, payload?.download, payload?.download_url, payload?.url
  ];
  for (const value of candidates) {
    const url = publicHttpsUrl(value);
    if (url && !hostMatches(new URL(url).hostname, ["youtube.com", "youtu.be"])) return url;
  }
  const collected = collectDownloadItems(payload);
...
function normalizeYoutube(payload, officialUrl, source = {}) {
  if (source.metadataOnly) return normalizeOfficialMetadata("youtube", payload, officialUrl);
  const meta = metadataSource(payload);
  const url = youtubeDirectUrl(payload);
  if (!url) throw Object.assign(new Error("Provider YouTube tidak mengembalikan file yang dapat diunduh."), { status: 422, code: "EMPTY_PROVIDER_RESULT" });
  const type = String(source.mediaType || meta.format || meta.type || "MP4").toUpperCase() === "MP3" ? "MP3" : "MP4";
  const title = cleanText(meta.title || meta.name || payload?.title || "YouTube Media", 160);
  const safeBase = title.replace(/[^\p{L}\p{N}._ -]+/gu, "_").trim() || "youtube_media";
  return {
    title,
    author: cleanText(meta.author || meta.author_name || meta.channel || payload?.author_name || payload?.author),
    caption: "",
    thumbnail: publicHttpsUrl(meta.thumbnail || meta.thumbnail_url || payload?.thumbnail_url || payload?.thumbnail),
    duration: numberValue(meta.duration || payload?.duration),
    stats: {},
    media: [{
      id: type === "MP3" ? "audio" : "video",
      type,
      url,
      filename: `${safeBase}.${type === "MP3" ? "mp3" : "mp4"}`,
      quality: cleanText(meta.quality || meta.resolution || (type === "MP3" ? "Audio MP3" : "Video MP4"), 40),
      size: numberValue(meta.size || meta.filesize),
      thumbnail: publicHttpsUrl(meta.thumbnail || meta.thumbnail_url || payload?.thumbnail_url || payload?.thumbnail)
    }],
    officialUrl,
    notice: "File diambil melalui provider server-side. Hanya media publik yang dapat diproses."
  };
}
...
function normalizeOfficialMetadata(provider, payload, officialUrl) {
  return {
    ...
    notice: provider === "youtube"
      ? "Provider file YouTube sedang tidak tersedia. Metadata dan tautan resmi YouTube tetap ditampilkan sebagai fallback."
      : "Provider audio Spotify sedang tidak tersedia. Metadata dan tautan resmi Spotify tetap ditampilkan sebagai fallback."
  };
}

function normalizeProvider(provider, payload, officialUrl, source = {}) {
  ...
  if (provider === "youtube") return normalizeYoutube(payload, officialUrl, source);
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    try {
      for (let redirects = 0; redirects <= MAX_REDIRECTS; redirects += 1) {
        ...
        const response = await (options.fetch || fetch)(current, { method: "GET", redirect: "manual", cache: "no-store", signal: controller.signal, headers: { Accept: "application/json", "User-Agent": "All-Tools-Nexora/6.4.0" } });
...
async function resolveYoutubeProvider(inputUrl, context = {}, options = {}) {
  const key = responseCacheKey("youtube", inputUrl, context);
  const cached = options.cache === false ? null : cachedResponse(key);
  if (cached) return { ...cached, cache: "hit" };

  const sources = Array.isArray(options.sources) && options.sources.length ? options.sources : providerSources("youtube");
  const mediaSources = sources.filter((source) => !source.metadataOnly);
  const metadataSources = sources.filter((source) => source.metadataOnly);
  const failures = [];
  const pieces = [];

  // MP4 and MP3 are independent upstream jobs. Run them together so a slow
  // audio endpoint does not make the user wait for the video request first.
  const mediaResults = await Promise.all(mediaSources.map(async (source) => {
    if (!options.ignoreCircuit && isCircuitOpen("youtube", source)) return { source, failure: "CIRCUIT_OPEN" };
    try {
      const payload = await fetchSourceJson("youtube", source, inputUrl, context, { ...options, attempts: options.sourceAttempts || 1 });
      if (providerRejectedPayload(payload)) throw Object.assign(new Error(cleanText(payload.message || payload.msg || "Provider menolak URL.")), { status: 422, code: "UPSTREAM_REJECTED" });
      const data = normalizeYoutube(payload, inputUrl, source);
      markSourceSuccess("youtube", source);
      return { source, data };
    } catch (error) {
      if (Number(error.status) >= 500 || error.code === "EMPTY_PROVIDER_RESULT" || error.code === "UPSTREAM_REJECTED") markSourceFailure("youtube", source);
      return { source, failure: cleanText(error.code || "UPSTREAM_FAILED", 80) };
    }
  }));

  for (const result of mediaResults) {
    if (result.data) pieces.push({ source: result.source, data: result.data });
    else failures.push({ source: result.source.id, code: result.failure || "UPSTREAM_FAILED" });
  }

  if (pieces.length) {
    const first = pieces[0].data;
    const seen = new Set();
    const media = pieces.flatMap((piece) => piece.data.media || []).filter((item) => item.url && !seen.has(`${item.type}:${item.url}`) && seen.add(`${item.type}:${item.url}`));
    const data = {
      title: pieces.find((piece) => piece.data.title)?.data.title || first.title || "YouTube Media",
      author: pieces.find((piece) => piece.data.author)?.data.author || "",
      caption: "",
      thumbnail: pieces.find((piece) => piece.data.thumbnail)?.data.thumbnail || "",
      duration: pieces.find((piece) => piece.data.duration)?.data.duration || 0,
      stats: {},
      media,
      officialUrl: inputUrl,
      notice: media.length > 1 ? "MP4 dan MP3 siap diunduh melalui proxy Nexora." : "Satu format tersedia sekarang; format lain akan dicoba lagi pada permintaan berikutnya."
    };
    const result = { data, source: pieces.map((piece) => piece.source.id).join("+"), attempts: failures.length + pieces.length, cache: "miss" };
    cacheResponse(key, result, RESPONSE_CACHE_TTL_MS);
    return result;
  }

  for (const source of metadataSources) {
    if (!options.ignoreCircuit && isCircuitOpen("youtube", source)) continue;
    try {
      const payload = await fetchSourceJson("youtube", source, inputUrl, context, { ...options, attempts: 1 });
      const data = normalizeOfficialMetadata("youtube", payload, inputUrl);
      data.notice = "Provider MP4/MP3 sedang tidak tersedia. Metadata YouTube tetap berhasil dimuat; coba lagi beberapa saat.";
      markSourceSuccess("youtube", source);
      const result = { data, source: source.id, attempts: failures.length + 1, cache: "miss" };
      cacheResponse(key, result, 20_000);
      return result;
    } catch (error) {
  ...
  throw Object.assign(new Error(timedOut ? "Provider YouTube melewati batas waktu." : "Provider YouTube sedang gagal atau media tidak tersedia."), { status: timedOut ? 504 : 502, code: timedOut ? "UPSTREAM_TIMEOUT" : "UPSTREAM_ALL_FAILED", failures });
}

async function resolveProvider(provider, inputUrl, context = {}, options = {}) {
  if (provider === "youtube") return resolveYoutubeProvider(inputUrl, context, options);
  ...
  for (const source of sources) {
    ...
    try {
      ...
      const data = normalizeProvider(provider, payload, inputUrl, source);
...
async function providerHealth(provider) {
  ...
  // The outer tool-health probe times out at 4.5s by default. Waiting for every
  // upstream here with Promise.all + a 5s timeout guaranteed false negatives:
  // one slow backup could make an otherwise healthy provider exceed 4.5s.
  // Resolve as soon as ANY provider is healthy and cancel the remaining probes.
  ...
  return new Promise((resolve) => {
    const finish = (value) => {
      if (settled) return;
      settled = true;
      controllers.forEach((controller) => { try { controller.abort(); } catch {} });
      controllers.clear();
      resolve(value);
    };
    ...
    targets.forEach((value) => {
      (async () => {
        ...
        try {
          const result = await fetch(target, { method: "GET", redirect: "manual", cache: "no-store", signal: controller.signal, headers: { Accept: "application/json", Range: "bytes=0-2047", "User-Agent": "All-Tools-Nexora/6.4.0" } });
...
function downloaderMediaPath(provider, inputUrl, item, index) {
  const params = new URLSearchParams();
  params.set("provider", provider);
  params.set("download", "1");
  params.set("url", inputUrl);
  params.set("type", cleanText(item.type || "FILE", 12));
  params.set("index", String(index));
  params.set("filename", cleanText(item.filename || `nexora_${provider}_${index + 1}`, 120));
  return `/api/downloader?${params.toString()}`;
}

function publicDownloaderData(provider, inputUrl, data) {
  if (!data || !Array.isArray(data.media) || !["youtube", "spotify"].includes(provider)) return data;
  return {
    ...data,
    media: data.media.map((item, index) => ({
      ...item,
      url: item.url ? downloaderMediaPath(provider, inputUrl, item, index) : ""
    }))
  };
}

async function handleDownloaderFile(request, response, requestUrl, provider) {
  let inputUrl;
  try { inputUrl = validateProviderUrl(provider, requestUrl.searchParams.get("url")); }
  catch (error) { return send(response, error.status || 400, { ok: false, error: error.code || "INVALID_URL", message: error.message }); }
  const desiredType = cleanText(requestUrl.searchParams.get("type") || "", 12).toUpperCase();
  const requestedIndex = Math.max(0, Math.min(20, Number(requestUrl.searchParams.get("index")) || 0));
  try {
    const result = await resolveProvider(provider, inputUrl, {}, { cache: true });
    const downloadable = result.data.media.filter((item) => item && item.url && item.downloadable !== false);
    let item = downloadable[requestedIndex];
    if (desiredType) item = downloadable.find((candidate) => String(candidate.type || "").toUpperCase() === desiredType) || item;
    if (!item || !item.url) return send(response, 422, { ok: false, error: "FORMAT_NOT_AVAILABLE", message: "Format download sedang tidak tersedia. Coba proses link lagi." });
    return handleResolvedMedia(request, response, {
      url: item.url,
      filename: requestUrl.searchParams.get("filename") || item.filename,
      type: item.type || desiredType || "FILE",
      provider,
      probe: requestUrl.searchParams.get("probe") === "1" || request.method === "HEAD"
    });
  } catch (error) {
    const status = Number(error.status) || 502;
    return send(response, status, { ok: false, error: cleanText(error.code || "DOWNLOADER_FILE_FAILED", 80), message: cleanText(error.message || "File downloader gagal disiapkan.") });
  }
}
...
async function handleDownloader(request, response, requestUrl) {
  ...
  if ((request.method === "GET" || request.method === "HEAD") && requestUrl.searchParams.get("download") === "1") {
    if (!["youtube", "spotify"].includes(provider)) return send(response, 400, { ok: false, error: "DOWNLOAD_ROUTE_UNSUPPORTED", message: "Route file hanya tersedia untuk YouTube dan Spotify." });
    return handleDownloaderFile(request, response, requestUrl, provider);
  }
  ...
  try {
    ...
    const data = publicDownloaderData(provider, inputUrl, result.data);
    return send(response, 200, { ok: true, provider, capability, formats, source: result.source, attempts: result.attempts, cache: result.cache, data });
  } catch (error) {
    const status = Number(error.status) || 502;

...
function safeConfigName(value, fallback) {
...
function getConfig() {
  ...
  const modeRaw = cleanText(process.env.ALIGHT_PREMIUM_API_KEY_MODE || "both", 16).toLowerCase();
  const apiKeyMode = ["query", "header", "both"].includes(modeRaw) ? modeRaw : "both";
  return {
    baseUrl,
    apiKey: normalizeApiKey(process.env.ALIGHT_PREMIUM_API_KEY),
    apiKeyMode,
    queryName: safeConfigName(process.env.ALIGHT_PREMIUM_API_KEY_QUERY, "apikey"),
    headerName: safeConfigName(process.env.ALIGHT_PREMIUM_API_KEY_HEADER, "x-api-key").toLowerCase(),
...
function requestApiKey(request, config) {
  const supplied = normalizeApiKey(request.headers["x-nexora-alight-key"]);
  const key = supplied || config.apiKey;
  if (!key) {
    throw Object.assign(new Error("API key belum diisi. Masukkan API key di tool atau set ALIGHT_PREMIUM_API_KEY di Vercel."), {
      status: 503,
      code: "ALIGHT_PREMIUM_NOT_CONFIGURED"
    });
  }
  return key;
}
...
async function callProvider({ action, email, link, apiKey, config }) {
  ...
  const headers = { Accept: "application/json, text/plain;q=0.9, */*;q=0.8", "User-Agent": "All-Tools-Nexora-Alight/1.0" };
  if (config.apiKeyMode === "query" || config.apiKeyMode === "both") url.searchParams.set(config.queryName, apiKey);
  if (config.apiKeyMode === "header" || config.apiKeyMode === "both") headers[config.headerName] = apiKey;

  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), config.timeoutMs);
  try {
    const upstream = await fetch(url, { method: "GET", headers, redirect: "follow", cache: "no-store", signal: controller.signal });
    ...
    if (!upstream.ok) {
      let message = providerMessage(data, `Provider mengembalikan HTTP ${upstream.status}.`);
      if (upstream.status === 401 || upstream.status === 403) message = "API key ditolak oleh provider. Periksa key atau mode autentikasi.";
...
async function handleAlightPremium(request, response) {
  ...
  try {
    ...
    const config = getConfig();
    const apiKey = requestApiKey(request, config);
    ...
    const result = await callProvider({ action, email, link, apiKey, config });

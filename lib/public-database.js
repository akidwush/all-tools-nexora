...
const RESOURCES = {
  tools: {
    // The browser merges these rows with the bundled catalogue. Returning the
    // active flag prevents a stale database from hiding newly deployed tools,
    // while still allowing an administrator to explicitly disable a tool.
    path: "tools?select=id,name,description,category,badge,icon,external_url,sort_order,is_active,metadata,access_level&order=sort_order.asc,name.asc",
...
const BUILTIN_TOOL_SEEDS = {
  ...
  bmkg: {
    id: "bmkg", name: "BMKG Indonesia",
    description: "Gempa terkini, prakiraan cuaca 3 hari dan peringatan dini cuaca dari Open Data BMKG",
    category: "tools", badge: "BMKG", icon: "fa-solid fa-cloud-sun-rain", external_url: null,
    is_active: true, sort_order: 66,
    metadata: { provider: "bmkg-open-data", apiKeyRequired: false, attribution: "Sumber: BMKG", features: ["earthquake", "weather", "nowcast"] }
  }
};

function publicToolShape(item) {
...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    const rows = await databaseRequest(definition.path, { method: "GET" });
    const data = resource === "tools" && Array.isArray(rows)
      ? rows.filter((item) => !RETIRED_TOOL_IDS.has(item.id)).map(publicToolShape)
      : rows;

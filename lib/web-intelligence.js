...
const { assertPublicUrl, createPinnedLookup, isBlockedIp, validateUrlSyntax } = require("./audit");
...
function buildChecks(context) {
  ...
  const rows = [
    ...
    check("sec-csp", "security", "Content Security Policy", csp ? (csp.split(/;\s*/).some((directive) => directive.trim().toLowerCase().startsWith("script-src ") && directive.trim().split(/\s+/).includes("'unsafe-eval'")) ? "warn" : "pass") : "fail", 16, csp ? (csp.split(/;\s*/).some((directive) => directive.trim().toLowerCase().startsWith("script-src ") && directive.trim().split(/\s+/).includes("'unsafe-eval'")) ? "CSP memakai unsafe-eval" : "CSP aktif") : "Header CSP tidak ada", "Pasang CSP yang membatasi script, frame, object, dan koneksi eksternal."),
...
async function handleWebIntelligence(request, response, requestUrl) {
  if ((request.method === "GET" || request.method === "HEAD") && requestUrl.searchParams.get("health") === "1") {
    return send(response, 200, { ok: true, status: "ready", engine: "nexora-web-intelligence", engineVersion: "1.0.1", deepScan: true, pageSpeedKeyConfigured: Boolean(String(process.env.GOOGLE_PAGESPEED_API_KEY || "").trim()), safeBrowsingKeyConfigured: Boolean(String(process.env.GOOGLE_SAFE_BROWSING_API_KEY || "").trim()) }, request.method === "HEAD");
  }
  ...
  try {
    ...
    if (!inFlight.has(cacheKey)) {
      assertCapacity(inFlight, 32, "WEB_INTELLIGENCE_BUSY");
      inFlight.set(cacheKey, scanWebsite(normalized, mode).finally(() => inFlight.delete(cacheKey)));
    }
    const payload = await inFlight.get(cacheKey);
    const savedAt = Date.now();
    setBounded(cache, cacheKey, { payload, savedAt }, {
      maxEntries: 128,
      now: savedAt,
      isExpired: (entry) => savedAt - entry.savedAt >= cacheMs
    });

  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        "User-Agent": "All-Tools-Nexora-Crypto/6.3.14",
        ...(options.headers || {})
      }
    });
    if (!response.ok) {
...
function checkRateLimit(request) {
  ...
  if (visitors.size > 2_000) {
    for (const [ip, entry] of visitors) if (now - entry.startedAt >= RATE_WINDOW_MS) visitors.delete(ip);
  }
  return { allowed: row.count <= RATE_LIMIT, retryAfter: Math.max(1, Math.ceil((RATE_WINDOW_MS - (now - row.startedAt)) / 1000)) };
}

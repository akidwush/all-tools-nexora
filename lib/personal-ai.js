...
function normalizeSettings(input = {}) {
  ...
  if (!ALLOWED_MODELS.includes(modelCandidate)) throw validationError("Model Nexora AI tidak termasuk daftar yang diizinkan.", "INVALID_AI_MODEL");
...
async function generateGeminiReply({ settings, message, history = [], clientFactory, timeoutMs = 22000 }) {
  ...
  if (!apiKey) {
    const error = new Error("Nexora AI belum dikonfigurasi.");
  ...
  for (const model of modelCandidates) {
    ...
    try {
      ...
      if (!text) {
        const blocked = new Error("Nexora AI tidak menghasilkan respons.");
  ...
  throw lastError || providerError(Object.assign(new Error("Nexora AI timeout"), { name: "AbortError" }));
}

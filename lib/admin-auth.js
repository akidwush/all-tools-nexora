...
function verifyMutationRequest(request) {
  const cookies = parseCookies(request);
  const csrfCookie = String(cookies[CSRF_COOKIE] || "");
  ...
  if (!csrfCookie || !csrfHeader || csrfCookie.length !== csrfHeader.length) return false;
  if (!crypto.timingSafeEqual(Buffer.from(csrfCookie), Buffer.from(csrfHeader))) return false;

...
const { enforceToolControlPlane } = require("./admin-control-plane");
...
async function authorizeTool(request, response, toolId) {
  ...
  if (!(await enforceToolControlPlane(request, response, toolId))) return false;
...
function readerPublicConfig() {
  const config = getDatabaseConfig();
  let publicKey = config.publicKey || "";
  // Reject a misplaced privileged key, including a legacy service-role JWT.
  if (!publicKey.startsWith("sb_publishable_")) {
    try { if (JSON.parse(Buffer.from(publicKey.split(".")[1], "base64url").toString()).role !== "anon") publicKey = ""; }
    catch { publicKey = ""; }
  }
  const configured = Boolean(config.validUrl && publicKey);
  return { configured, url: configured ? config.url : "", publicKey: configured ? publicKey : "" };
}
...
async function handleAccount(request, response) {
  ...
  try {
    if (request.method === "GET" && new URL(request.url || "/", "https://nexora.invalid").searchParams.get("resource") === "reader-config") {
      const config = readerPublicConfig();
      return send(response, 200, { ok: true, ...config });
    }
    ...
    const session = await requireAuthenticatedUser(request, response);
    if (!session) return;
    if (action === "reader-session") {
      response.setHeader("Cache-Control", "private, no-store");
      return send(response, 200, { ok: true, ...readerPublicConfig(), userId: session.user.id, accessToken: session.access });
    }
    if (action === "profile") {

...
function origin(req){const proto=String(req.headers["x-forwarded-proto"]||"").split(",")[0].trim()||(req.socket?.encrypted?"https":"http");const host=String(req.headers["x-forwarded-host"]||req.headers.host||"").split(",")[0].trim();return host?`${proto}://${host}`:"";}
...
async function handleAccount(req,res){
  ...
  try{
    ...
    if(["login","register","forgot","adopt-session"].includes(action)){
      if(action==="login"){const session=await signInWithPassword(clean(body.email,254).toLowerCase(),String(body.password||""));setCookies(req,res,session);return send(res,200,publicSession({user:session.user,access:session.access_token,...await membership(session.user)}));}
      if(action==="register"){const email=clean(body.email,254).toLowerCase(),password=String(body.password||"");if(password.length<8)return send(res,400,{ok:false,error:"WEAK_PASSWORD",message:"Password minimal 8 karakter."});const data=await authRequest("/auth/v1/signup",{method:"POST",body:{email,password,data:{display_name:clean(body.displayName,80)}}});if(data.access_token){setCookies(req,res,data);return send(res,201,publicSession({user:data.user,access:data.access_token,...await membership(data.user)}));}return send(res,201,{ok:true,authenticated:false,confirmationRequired:true,message:"Periksa email untuk mengonfirmasi akun."});}
      if(action==="forgot"){const redirect=`${origin(req)}/?account=recovery`;await authRequest(`/auth/v1/recover?redirect_to=${encodeURIComponent(redirect)}`,{method:"POST",body:{email:clean(body.email,254).toLowerCase()}});return send(res,200,{ok:true,message:"Tautan pemulihan telah dikirim jika email terdaftar."});}
    ...
    if(action==="reset-password"){const password=String(body.password||"");if(password.length<8)return send(res,400,{ok:false,error:"WEAK_PASSWORD",message:"Password minimal 8 karakter."});await authRequest("/auth/v1/user",{method:"PUT",accessToken:session.access,body:{password}});return send(res,200,{ok:true,message:"Password berhasil diperbarui."});}
    return send(res,400,{ok:false,error:"INVALID_ACTION"});
  }catch(error){const status=Math.max(400,Math.min(599,Number(error.status||500)));console.error("[account]",error.code||"UNKNOWN");return send(res,status,{ok:false,error:error.code||"ACCOUNT_FAILED",message:status>=500?"Layanan akun belum dapat diproses.":clean(error.message,300)});}
}

async function authorizeTool(req,res,toolId){let rows;try{rows=await databaseRequest(`tools?select=access_level&id=eq.${encodeURIComponent(toolId)}&limit=1`,{method:"GET"});}catch(error){if(error?.code==="DATABASE_NOT_CONFIGURED")return true;throw error;}if(rows?.[0]?.access_level!=="vvip")return true;const session=await resolve(req,res);if(!session){send(res,401,{ok:false,error:"VVIP_LOGIN_REQUIRED",message:"Login dan membership VVIP diperlukan."});return false;}if(!session.membership.isVvip){send(res,403,{ok:false,error:session.membership.status==="suspended"?"ACCOUNT_SUSPENDED":"VVIP_REQUIRED",message:"Tool ini khusus member VVIP aktif."});return false;}return true;}

...
const MAX_ARCHIVE_BYTES = 3_600_000;
...
function netlifyHeaders(token, extra = {}) {
  return { Authorization: `Bearer ${token}`, 'User-Agent': 'All-Tools-Nexora-Deploy/6.3.15', ...extra };
}
...
function decodeArchive(value) {
  ...
  if (!archive.length || archive.length > MAX_ARCHIVE_BYTES) throw new Error('Ukuran ZIP melebihi batas 3,6 MB.');
  return archive;
}
...
module.exports = { handleVDeploy, extractZip, projectName };

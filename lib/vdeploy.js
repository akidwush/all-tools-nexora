...
function requireAccess(req) {
  const expected = String(process.env.NEXORA_DEPLOY_ACCESS_KEY || '').trim();
  if (!expected) return { ok: false, status: 503, code: 'deploy_access_not_configured', message: 'Nexora Deploy belum dikonfigurasi di server.' };
  const supplied = String(req.headers['x-deploy-key'] || '').trim();
  if (!supplied || !safeEqual(supplied, expected)) {
    return { ok: false, status: 401, code: 'access_key_required', message: 'Kode akses Nexora Deploy diperlukan.' };
  }
  return { ok: true };
}
...
function netlifyHeaders(token, extra = {}) {
  return { Authorization: `Bearer ${token}`, 'User-Agent': 'All-Tools-Nexora-Deploy/6.4.0', ...extra };
}

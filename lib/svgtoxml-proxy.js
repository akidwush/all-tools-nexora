...
async function handleSvgToXml(request, response, requestUrl) {
  if (request.method === "GET") {
    try {
      ...
      if (!apiKey || !verify) {
        return send(response,200,{ok:true,configured:Boolean(apiKey),authorized:null,service:"svgtoxml-alight",baseUrl,modes:["optimized","lossless","accurate","balanced","lightweight"]});
      }
      const auth = await requestUpstream("/api/v1/auth",{method:"GET"});
      return send(response,200,{ok:true,configured:true,authorized:auth?.ok !== false,service:"svgtoxml-alight",baseUrl,modes:["optimized","lossless","accurate","balanced","lightweight"]});
    } catch(error) {

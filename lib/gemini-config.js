...
function geminiResponseText(response) {
  let direct = "";
  try {
    direct = [response?.text, response?.response?.text]
  ...
  const candidates = Array.isArray(response?.candidates)
    ? response.candidates
    : Array.isArray(response?.response?.candidates) ? response.response.candidates : [];
...
function geminiResponseDiagnostics(response) {
  const candidates = Array.isArray(response?.candidates)
    ? response.candidates
    : Array.isArray(response?.response?.candidates) ? response.response.candidates : [];
  ...
  return {
    ...
    finishReasons: [...new Set(candidates.map((candidate) => cleanModel(candidate?.finishReason)).filter(Boolean))],
    blockReason: cleanModel(feedback?.blockReason)
  };
}

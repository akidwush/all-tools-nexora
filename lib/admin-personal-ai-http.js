...
function send(response, status, payload) { response.setHeader("Cache-Control", "no-store, max-age=0"); response.setHeader("Content-Type", "application/json; charset=utf-8"); return response.status(status).json(payload); }
...
function adminShape(row) {
  ...
  return { published, draft, status: { apiConfigured: Boolean(String(process.env.GEMINI_API_KEY || "").trim()), published: Boolean(row && row.published_at), enabled: Boolean(row && row.published_at && row.enabled), publishedAt: row?.published_at || null, updatedAt: row?.updated_at || null }, allowedModels: ALLOWED_MODELS };
}
...
module.exports = async function handleAdminPersonalAi(request, response) {
  ...
  try {
    ...
    if (request.method === "POST") {
      ...
      if (!String(process.env.GEMINI_API_KEY || "").trim()) return send(response, 503, { ok: false, error: "GEMINI_NOT_CONFIGURED", message: "Gemini API belum dikonfigurasi." });

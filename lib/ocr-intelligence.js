...
const { assertCapacity, pruneMap } = require("./memory-store");
...
function checkRateLimit(request, now = Date.now()) {
  ...
  pruneMap(visitors, { now, maxEntries: 2_000, isExpired: (entry) => entry.dayResetAt <= now });
...
async function processDocument(payload, runtime = {}) {
  ...
  if (!inFlight.has(hash)) {
    assertCapacity(inFlight, 24, "OCR_SERVER_BUSY");

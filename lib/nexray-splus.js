...
const IMAGE_ENGINES = Object.freeze(["auto", "nanobanana"]);
...
function multipartBody(decoded, textField, textValue, fileField = "image") {
  const boundary = `----NexoraSPlus${crypto.randomBytes(12).toString("hex")}`;
  const ext = decoded.mime === "image/png" ? "png" : decoded.mime === "image/webp" ? "webp" : "jpg";
  const head = Buffer.from(
    `--${boundary}\r\n` +
    `Content-Disposition: form-data; name="${fileField}"; filename="nexora-splus.${ext}"\r\n` +
  ...
  const body = Buffer.concat([head, decoded.buffer, middle, tail]);
  return { boundary, body };
}

async function nanoBananaAttempt(decoded, param, runtime, variant) {
  const spec = variant === "prompt"
    ? { textField: "prompt", fileField: "image" }
    : variant === "file"
      ? { textField: "param", fileField: "file" }
      : { textField: "param", fileField: "image" };
  const multipart = multipartBody(decoded, spec.textField, param, spec.fileField);
  return fetchPayload(new URL("/ai/nanobanana", BASE_URL).toString(), {
    method: "POST",
    headers: {
...
async function editWith(engine, input, runtime) {
  if (engine !== "nanobanana") {
    throw error("NEXRAY_SPLUS_ENGINE_DISABLED", 410, "GPT Image S+ dinonaktifkan karena upstream Nexray saat ini merespons 401.");
  }
  ...
  // Nexray docs currently specify multipart fields image + param. The extra
  // variants are compatibility retries for provider-side parser drift.
  for (const variant of ["documented", "prompt", "file"]) {
    try {
      const payload = await nanoBananaAttempt(decoded, param, runtime, variant);
      const imageUrl = payload.directDataUrl || findUrl(payload, "image");
      if (!imageUrl) throw error("NEXRAY_SPLUS_EMPTY_RESULT", 502, "Nano Banana tidak mengembalikan gambar hasil.");
      return { provider: "Nexray Nano Banana", engine: "nanobanana", imageUrl, transport: variant, attempts };
    } catch (e) {
      lastError = e;
      attempts.push(variant);
      if (Number(e?.status) === 429 || Number(e?.status) >= 500) break;
    }
  }

  const e = lastError || error("NEXRAY_SPLUS_NANO_FAILED", 502, "Nano Banana sedang tidak tersedia.");
  e.attempts = attempts;
  throw e;
}
...
async function imageEdit(input, runtime) {
  ...
  if (requested === "gptimage") {
    throw error("NEXRAY_SPLUS_ENGINE_DISABLED", 410, "GPT Image S+ dinonaktifkan karena upstream Nexray saat ini merespons 401.");
  }
  return editWith("nanobanana", input, runtime);
}

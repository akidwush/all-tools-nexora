...
async function comicPages(requestUrl, dependencies) {
  ...
  return { data: { image: files.map((file) => `${baseUrl}/${folder}/${hash}/${encodeURIComponent(file)}`), quality: useFull ? "full" : "saver" }, source: "MangaDex" };
}

function httpsBufferWithPinnedDns(url, timeoutMs = 14_000) {
  return resolveWithSecureDns(url.hostname, Math.min(5000, timeoutMs)).then((addresses) => new Promise((resolve, reject) => {
    ...
    const request = https.request(url, { method: "GET", lookup, servername: url.hostname, headers: { Accept: "image/avif,image/webp,image/jpeg,image/*", "User-Agent": "All-Tools-Nexora-Comic-Reader/1.0 (+https://all-tools-nexora.vercel.app)" } }, (upstream) => {
      const chunks = [];
      let bytes = 0;
      upstream.on("data", (chunk) => {
        bytes += chunk.length;
        if (bytes > 3_000_000) return request.destroy(Object.assign(new Error("Sampul terlalu besar."), { code: "COMIC_COVER_TOO_LARGE", status: 502 }));
        chunks.push(Buffer.from(chunk));
      });
      upstream.on("end", () => {
        if (upstream.statusCode < 200 || upstream.statusCode >= 300) return reject(Object.assign(new Error("Sampul tidak ditemukan."), { code: "COMIC_COVER_NOT_FOUND", status: 404 }));
        resolve({ buffer: Buffer.concat(chunks), contentType: clean(upstream.headers["content-type"], 100) || "image/jpeg" });
      });
    });
    request.setTimeout(timeoutMs, () => request.destroy(Object.assign(new Error("Sampul melewati batas waktu."), { code: "COMIC_TIMEOUT", status: 504 })));
    request.on("error", reject);
    request.end();
  }));
}
...
async function handleComicReader(request, response, dependencies = {}) {
  ...
  try {
    ...
    const action = queryValue(requestUrl, "action", 30).toLowerCase();
    if (!action) return send(response, 200, { success: true, status: "ready", provider: "MangaDex", privacy: "no-account-required" }, request.method === "HEAD");
    const bucket = takeFixedWindow(requestBuckets, requestIp(request), { windowMs: 60_000, limit: 60, maxEntries: 5000 });
    if (!bucket.allowed) {
    ...
    let result;
    if (["list", "search"].includes(action)) result = await listComics(requestUrl, dependencies);
    else if (action === "detail") result = await comicDetail(requestUrl, dependencies);
    else if (action === "chapters") result = await comicChapters(requestUrl, dependencies);
    else if (action === "pages") result = await comicPages(requestUrl, dependencies);
    else throw inputError("Aksi komik tidak dikenal.", "COMIC_ACTION_INVALID");
    ...
    return send(response, status, { success: false, error: error?.code || "COMIC_FAILED", message: error?.message || "Fitur komik mengalami gangguan." }, request.method === "HEAD");
  }
}

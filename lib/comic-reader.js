...
async function publicRuntimeSourceRegistry(registry) {
  const output = [];
  for (const source of publicSourceRegistry(registry)) {
    ...
    if (config.mode === "disabled") continue;
    output.push({ ...source, enabled: config.mode === "active", maintenance: config.mode === "maintenance" });
  }
  return output;
}
...
async function handleComicReader(request, response, dependencies = {}) {
  ...
  try {
    ...
    if (!action) return send(response, 200, { success: true, status: "ready", provider: "MangaDex", source: "mangadex", sources: await publicRuntimeSourceRegistry(registry), privacy: "no-account-required" }, request.method === "HEAD");
    if (action === "sources") return send(response, 200, { success: true, sources: await publicRuntimeSourceRegistry(registry) }, request.method === "HEAD");

...
const { endpointDefinition: mangaDexEndpointDefinition } = require("./comic-sources/mangadex");
const { createComicSourceRegistry, isAllowedImageHost, publicSourceRegistry, recordSourceHealth, resetSourceHealth, sourceClassification } = require("./comic-sources/registry");
...
async function fetchMangaDex(path, { fetchImpl = globalThis.fetch, timeoutMs = 14_000, cacheMs = 60_000 } = {}) {
  ...
  const endpoint = await resolveProviderConfig(mangaDexEndpointDefinition);
  const apiBase = new URL(endpoint.baseUrl);
  const url = new URL(path, apiBase);
  if (url.origin !== apiBase.origin) throw inputError("Tujuan MangaDex tidak valid.", "COMIC_UPSTREAM_INVALID");
  timeoutMs = Number.isFinite(Number(timeoutMs)) && Number(timeoutMs) > 0
    ? Math.min(Number(timeoutMs), endpoint.timeoutMs)
    : endpoint.timeoutMs;
...
async function dispatchSourceAction(adapter, action, requestUrl, dependencies) {
  const source = adapter.definition.id;
  if (action === "health") {
    if (typeof adapter.health !== "function") throw inputError("Health check provider tidak tersedia.", "UNSUPPORTED_CAPABILITY", 501);
    const health = await adapter.health({ fetchImpl: dependencies.fetchImpl, timeoutMs: dependencies.timeoutMs, signal: dependencies.signal });
    recordSourceHealth(source, { ok: health.status === "active", status: health.httpStatus || (health.status === "active" ? 200 : 503), latency: health.latencyMs || 0 });
    return { source, provider: adapter.definition.label, health, capabilities: adapter.definition.capabilities };
  }
...
async function publicRuntimeSourceRegistry(registry, options = {}) {
  const scope = options.scope === "experimental" ? "experimental" : "standard";
  const includeDisabled = options.includeDisabled === true;
  const output = [];
  for (const source of publicSourceRegistry(registry, { scope })) {
    const adapter = registry.get(source.id);
    if (!adapter?.endpointDefinition) { output.push(source); continue; }
    const config = await resolveProviderConfig(adapter.endpointDefinition, { allowUnavailable: true });
    if (config.mode === "disabled" && !includeDisabled) continue;
    output.push({
      ...source,
      enabled: config.mode === "active" && adapter.definition.availability !== "unsupported",
      maintenance: config.mode === "maintenance",
      adminMode: config.mode,
      availability: adapter.definition.availability || "supported",
      unsupportedReason: adapter.definition.unsupportedReason || null
    });
  }
  return output;
}
...
async function handleComicReader(request, response, dependencies = {}) {
  ...
  try {
    ...
    const requestMode = queryValue(requestUrl, "mode", 30).toLowerCase();
    const sourceScope = queryValue(requestUrl, "scope", 30).toLowerCase();
    const registry = getComicSourceRegistry();
    if (!action) return send(response, 200, { success: true, status: "ready", provider: "MangaDex", source: "mangadex", sources: await publicRuntimeSourceRegistry(registry, { scope: "standard" }), privacy: "no-account-required" }, request.method === "HEAD");
    if (action === "sources") {
      if (sourceScope === "experimental") {
        // /api/comics is rewritten to /api/health?mode=comic-reader.
        // The rewrite can consume the generic mode query parameter, so
        // experimental source metadata is also explicitly identified by
        // scope=experimental. Actual experimental provider actions below
        // still require mode=experimental.
        if (requestMode !== "experimental" && requestMode !== "comic-reader") {
          throw inputError("Experimental source membutuhkan explicit experimental mode.", "COMIC_EXPERIMENTAL_OPT_IN_REQUIRED", 403);
        }
        return send(response, 200, { success: true, experimental: true, sources: await publicRuntimeSourceRegistry(registry, { scope: "experimental", includeDisabled: true }) }, request.method === "HEAD");
      }
      return send(response, 200, { success: true, sources: await publicRuntimeSourceRegistry(registry, { scope: "standard" }) }, request.method === "HEAD");
    }
    ...
    if (classification.experimental) {
      if (requestMode !== "experimental") throw inputError("Experimental source membutuhkan explicit opt-in flow.", "COMIC_EXPERIMENTAL_OPT_IN_REQUIRED", 403);
      const endpoint = adapter.endpointDefinition
        ? await resolveProviderConfig(adapter.endpointDefinition, { allowUnavailable: true })
        : { mode: adapter.definition.defaultEnabled === true ? "active" : "disabled" };
      if (endpoint.mode === "disabled") throw inputError("Provider experimental dinonaktifkan oleh administrator.", "COMIC_EXPERIMENTAL_DISABLED", 503);
      if (endpoint.mode === "maintenance") throw inputError("Provider experimental sedang maintenance.", "COMIC_EXPERIMENTAL_MAINTENANCE", 503);
      if (adapter.definition.availability === "unsupported") throw inputError(adapter.definition.unsupportedReason || "Provider experimental belum didukung.", "COMIC_EXPERIMENTAL_UNSUPPORTED", 503);
    }

...
const { getUtilityEndpointDefinition } = require("./external-endpoint-registry");
...
function providerError(status, payload) {
  ...
  if (status === 429 || /\brate\s*limit\b|\blimit(?:ed)?\b|\bquota\b|too many/.test(hint)) {
    error.code = "AI_SONG_RATE_LIMITED";
    error.category = "rate_limit";
  } else if ([401, 403].includes(status) || /api.?key|unauthor|forbidden|revoked/.test(hint)) {
    error.code = "AI_SONG_CONFIGURATION_ERROR";
    error.category = "authentication";
  } else if (status === 408 || /timed?\s*out|timeout/.test(hint)) {
    error.code = "AI_SONG_TIMEOUT";
    error.category = "timeout";
  } else if ([413, 422].includes(status) || /invalid\s+prompt|prompt.{0,30}(?:invalid|required|empty|too\s+(?:short|long)|tidak\s+valid|wajib|kosong)/.test(hint)) {
    error.code = "AI_SONG_PROVIDER_REJECTED";
    error.category = "input_rejected";
  } else {
    error.code = "AI_SONG_UPSTREAM_ERROR";
    error.category = status >= 500 ? "provider_server_error" : "provider_generation_error";
  }
  return error;
}
...
function statusForError(error) {
  ...
  if (error?.code === "AI_SONG_UPSTREAM_ERROR") return 503;
  return 502;
}
...
async function upstreamRequest(input, runtime = {}) {
  ...
  const endpoint = await resolveProviderConfig(getUtilityEndpointDefinition("utility:aisong"));
  ...
  const effectiveTimeoutMs = Number(runtime.timeoutMs) > 0 ? Math.min(Number(runtime.timeoutMs), endpoint.timeoutMs) : endpoint.timeoutMs;
  ...
  try {
    ...
    if (!upstream.ok || payload?.status === false || payload?.success === false || payload?.ok === false) {
      ...
      console.warn("[ai-song] upstream rejected request", {
        status: upstream.status,
        category: rejection.category,
        durationMs: Date.now() - startedAt,
        shape: responseShape(payload, apiKey)
      });
      throw rejection;
    }

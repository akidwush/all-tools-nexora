...
const IMAGE_KEYS = ["full_file_url", "fullFileUrl", "file_url", "fileUrl", "image_url", "imageUrl", "original_url", "originalUrl", "original", "file", "image", "url"];
...
async function upstreamRequest(query, mode, runtime = {}) {
  ...
  const endpoint = await resolveProviderConfig(getUtilityEndpointDefinition("utility:danbooru"));
  ...
  try {
    ...
    if (!normalized.items.length && !hasRecognizedCollection(payload)) {
      console.warn("[danbooru] unsupported response schema", responseShape(payload, apiKey));

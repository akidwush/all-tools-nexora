...
async function startProviderTask({ inputUrl, filename, options }) {
  ...
  if (!taskId || ["error", "failed", "param_error", "auth_error"].includes(status)) {
    ...
    throw createError(`Bigjpg menolak tugas (${status || "invalid_response"}).`, status ? `BIGJPG_${status.toUpperCase()}` : "BIGJPG_TASK_INVALID", category === "quota" ? 429 : category === "auth" ? 403 : 422, { category, provider: data });
  }

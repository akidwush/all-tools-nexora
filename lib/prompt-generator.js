...
function providerError(error) {
  ...
  if (status === 429 || /quota|rate limit|resource exhausted/.test(message)) return inputError("Layanan AI sedang sibuk atau kuotanya tercapai. Coba lagi sesaat.", "PROMPT_AI_RATE_LIMITED", 429);
  if (status === 401 || status === 403 || /api key|unauth|forbidden/.test(message)) return inputError("Gemini API belum dapat digunakan.", "PROMPT_AI_NOT_CONFIGURED", 503);
...
async function generatePrompt({ image, options, clientFactory, timeoutMs = 50_000 }) {
  ...
  if (!apiKey) throw inputError("GEMINI_API_KEY belum dikonfigurasi.", "PROMPT_AI_NOT_CONFIGURED", 503);
  ...
  try {
    const response = await client.models.generateContent({
      model: clean(process.env.PROMPT_GENERATOR_MODEL, 80) || clean(process.env.GEMINI_MODEL, 80) || DEFAULT_MODEL,
...
async function handlePromptGenerator(request, response, dependencies = {}) {
  ...
  if (request.method === "GET" || request.method === "HEAD") return send(response, 200, {
    ok: true, configured: Boolean(clean(process.env.GEMINI_API_KEY, 500)),
...
module.exports = { ALLOWED_MIME_TYPES, MAX_IMAGE_BYTES, handlePromptGenerator, normalizeOptions, parseImage, parsePromptResponse, resetPromptGeneratorState };

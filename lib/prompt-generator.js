...
async function generatePrompt({ image, options, clientFactory, timeoutMs = 50_000 }) {
  ...
  for (const [modelIndex, model] of modelCandidates.entries()) {
    ...
    const attemptTimeout = modelCandidates.length > 1 ? Math.min(modelIndex === 0 ? 42_000 : 20_000, remaining) : remaining;
    ...
    try {
      ...
      const canFallback = modelIndex < modelCandidates.length - 1 && ["PROMPT_AI_TIMEOUT", "PROMPT_AI_MODEL_UNAVAILABLE"].includes(lastError.code);

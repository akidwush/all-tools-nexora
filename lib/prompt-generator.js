  ...
  for (const [modelIndex, model] of modelCandidates.entries()) {
    ...
    try {
      ...
      if (!text) {
        ...
        if (blocked) throw inputError("Gambar ditolak oleh filter keamanan Gemini. Coba gambar referensi lain.", "PROMPT_AI_BLOCKED", 422);
        throw inputError("Gemini tidak menghasilkan prompt.", "PROMPT_AI_EMPTY_RESPONSE", 502);
      }

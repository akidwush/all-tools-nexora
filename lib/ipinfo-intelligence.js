...
async function handleIpIntelligence(request, response, url) {
  ...
  const rate = takeFixedWindow(requestBuckets, requestIp(request) || "unknown", {
    windowMs: 60_000,
    limit: 30,
    maxEntries: 2_000
  });
  if (!rate.allowed) {
    ...
    return send(response, 429, { ok: false, error: "IPINFO_RATE_LIMITED", message: "Terlalu banyak lookup IP. Coba lagi sesaat." });
  }

...
function requestPinnedBuffer(parsed, signal){
  ...
  return new Promise((resolve,reject)=>{
    ...
    const request=https.request(parsed,{
      ...
      headers:{Accept:"image/svg+xml,application/xml;q=0.9,*/*;q=0.1","Accept-Encoding":"identity","User-Agent":"All-Tools-Nexora-Vector/6.3.14"}
    },(response)=>{
...
async function proxySvg(taskIdValue,response){
  ...
  try{
    ...
    response.setHeader("X-Nexora-Vector-Pipeline","6.3.14-pinned-download");
...
module.exports={MAX_SVG_BYTES,downloadPublicSvg,handleFreeConvertVectorizer,normalizeFormat,findExportUrl};

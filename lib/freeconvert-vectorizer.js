...
async function freeConvert(path, options = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      error.status = response.status === 402 || response.status === 429 ? 429 : response.status === 401 || response.status === 403 ? response.status : 502;
...
async function startConversion(body) {
  ...
  // HF9.3:
  // FreeConvert's PNG/JPG -> SVG engine exposes converter-specific
  // numeric ranges. Auto-mapping UI values (0-100) can feed invalid
  // values such as segment_length=50 where the engine accepts 3.5-10.
  // Use FreeConvert's validated defaults until option ranges are mapped
  // explicitly per converter.

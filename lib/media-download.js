...
async function fetchAllowed(rawUrl, request, probe, controller) {
  let current = validateMediaUrl(rawUrl);
  for (let redirects = 0; redirects <= MAX_REDIRECTS; redirects += 1) {
    const upstream = await fetch(current, {
      method: "GET",
      headers: upstreamHeaders(request, probe, current),
      redirect: "manual",
      cache: "no-store",
...
async function handleMediaDownload(request, response, requestUrl) {
  ...
  try {
    const result = await fetchAllowed(mediaUrl, request, probe, controller);

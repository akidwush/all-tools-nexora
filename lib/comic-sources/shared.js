...
async function requestJson({
  ...
  if (endpointDefinition) {
    const effective = await resolveProviderConfig(endpointDefinition);
    base = effective.baseUrl;
    const requestedTimeout = Number(timeoutMs);
    timeoutMs = Number.isFinite(requestedTimeout) && requestedTimeout > 0
      ? Math.min(requestedTimeout, effective.timeoutMs)
      : effective.timeoutMs;
  }
...
module.exports = { absoluteHttpsUrl, buildUrl, clearSourceCache, error, positiveInt, requestJson, slug, token };

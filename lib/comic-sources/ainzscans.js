...
const endpointDefinition = Object.freeze({
  id: "comic:ainzscans", group: "comic", label: "Ainzscans", category: "comic",
  allowedHosts: ["api.ainzscans01.com"], defaultBaseUrl: definition.apiBase,
  defaultTimeoutMs: 10_000, minTimeoutMs: 1_000, maxTimeoutMs: 60_000,
  healthPath: "/api/search?type=COMIC&limit=1&page=1&sort=latest&order=desc", healthMethod: "GET",
...
async function listPage({ page = 1, limit = 24, query = "", fetchImpl, timeoutMs, signal }) {
  ...
  const payload = await requestJson({ endpointDefinition, base: definition.apiBase, path: "./search", params, provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 120_000 });
...
async function getManga({ id, fetchImpl, timeoutMs, signal }) {
  ...
  const payload = await requestJson({ endpointDefinition, base: definition.apiBase, path: `./series/comic/${encodeURIComponent(safe)}`, provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 300_000 });
  return mapManga(payload || {});
}
async function getChapters({ mangaId, fetchImpl, timeoutMs, signal }) {
  const safe = slug(mangaId, "Slug manga");
  const payload = await requestJson({ endpointDefinition, base: definition.apiBase, path: `./series/comic/${encodeURIComponent(safe)}`, provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 120_000 });
...
async function getPages({ mangaId, chapterId, fetchImpl, timeoutMs, signal }) {
  ...
  const payload = await requestJson({ endpointDefinition, base: definition.apiBase, path: `./series/comic/${encodeURIComponent(manga)}/chapter/${encodeURIComponent(chapter)}`, provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 300_000 });

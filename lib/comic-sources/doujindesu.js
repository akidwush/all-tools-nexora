...
const SECRET_SALT =
  "doujindesu-scrapers-cannot-read-this-super-secret-salt";
...
/* -------------------------------------------------------------------------- */
/* DoujinDesu response decryption                                             */
/* -------------------------------------------------------------------------- */
...
function hashKey(input) {
  ...
  let output = "";
  let value = Math.abs(hash) || 123456789;
...
function responseKeys() {
  ...
  return [
    hashKey(`${SECRET_SALT}_${hour}`),
    hashKey(`${SECRET_SALT}_${hour - 1}`),
    hashKey(`${SECRET_SALT}_${hour + 1}`)
  ];
}

function xorDecodeHex(payload, key) {
  ...
  for (let index = 0; index < payload.length; index += 2) {
    ...
    if (!Number.isFinite(byte)) {
      throw new Error("Invalid encrypted response byte.");
    }
  ...
  for (let index = 0; index < bytes.length; index += 1) {
    ...
    chars.push(
      String.fromCharCode(decoded & 255)
    );
...
function decryptResponsePayload(payload) {
  let lastError = null;

  for (const key of responseKeys()) {
    try {
      const decoded = xorDecodeHex(payload, key);
  ...
  throw providerError(
    `Gagal membuka respons terenkripsi DoujinDesu: ${
      lastError?.message || "payload tidak valid"
    }`,
    "UPSTREAM_DECRYPT_FAILED",
    502
  );
}
...
function unwrapResponsePayload(payload) {
  if (
    ...
    typeof payload._enc_resp_ === "string"
  ) {
    return decryptResponsePayload(payload._enc_resp_);
  }

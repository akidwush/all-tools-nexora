...
const endpointDefinition = Object.freeze({
  id: "comic:voratoon", group: "comic", label: "Voratoon", category: "comic",
  allowedHosts: ["api.voratoon.com"], defaultBaseUrl: definition.apiBase,
  defaultTimeoutMs: 10_000, minTimeoutMs: 1_000, maxTimeoutMs: 60_000,
  healthPath: "/series/?take=1&page=1", healthMethod: "GET",
...
function mapManga(raw) {
  ...
  return normalizeManga(definition.id, {
    ...
    coverUrl: absoluteHttpsUrl(row.coverImage || row.cover || row.cover_url || row.thumbnail || row.image, definition.website),
...
async function listPage({ page = 1, limit = 24, query = "", fetchImpl, timeoutMs, signal }) {
  ...
  const params = { take, takeChapter: 1, page: positiveInt(page, { max: 1000 }) };
  const q = text(query, 120);
  if (q) params.filter = `title=ilike="${q.replace(/["\\]/g, " ")}",nativeTitle=ilike="${q.replace(/["\\]/g, " ")}"`;
  else params.preset = "rilisan_terbaru";
  const payload = await requestJson({ endpointDefinition, base: definition.apiBase, path: "./", params, headers: headers(), provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 120_000 });
...
async function search({ query = "", page = 1, limit = 24, fetchImpl, timeoutMs, signal }) {
  ...
  const { rows, meta } = await listPage({ page, limit: requestedLimit, query, fetchImpl, timeoutMs, signal });
  const current = Number(meta.page || page) || page;
  const lastPage = Number(meta.lastPage || meta.totalPages || 0);
  return { items: rows.map(mapManga).slice(0, limit), hasMore: lastPage ? current < lastPage : rows.length >= requestedLimit, page };
}
async function getManga({ id, fetchImpl, timeoutMs, signal }) {
  const safe = slug(id, "Slug manga");
  const payload = await requestJson({ endpointDefinition, base: definition.apiBase, path: `./${encodeURIComponent(safe)}`, headers: headers(), provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 300_000 });
...
async function getChapters({ mangaId, fetchImpl, timeoutMs, signal }) {
  ...
  const payload = await requestJson({ endpointDefinition, base: definition.apiBase, path: `./${encodeURIComponent(safe)}/chapters`, headers: headers(), provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 120_000 });
  const rows = dataOf(payload);
...
async function getPages({ mangaId, chapterId, fetchImpl, timeoutMs, signal }) {
  ...
  const payload = await requestJson({ endpointDefinition, base: definition.apiBase, path: `./${encodeURIComponent(manga)}/chapters/${encodeURIComponent(chapter)}`, headers: headers(), provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 300_000 });

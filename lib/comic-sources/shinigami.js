...
const endpointDefinition = Object.freeze({
  id: "comic:shinigami", group: "comic", label: "Shinigami ID", category: "comic",
  allowedHosts: ["api.shngm.io"], defaultBaseUrl: definition.apiBase,
  defaultTimeoutMs: 10_000, minTimeoutMs: 1_000, maxTimeoutMs: 60_000,
  healthPath: "/v1/manga/list?page=1&page_size=1&type=project", healthMethod: "GET",
...
async function listPage({ page = 1, limit = 50, query = "", fetchImpl, timeoutMs, signal }) {
  ...
  if (q) params.q = q;
  const payload = await requestJson({ endpointDefinition, base: definition.apiBase, path: "./manga/list", params, headers: { Origin: "https://app.shinigami.asia", Referer: "https://app.shinigami.asia/" }, provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 120_000 });
...
async function getManga({ id, slug, titleHint, fetchImpl, timeoutMs, signal }) {
  ...
  try {
    const payload = await requestJson({ endpointDefinition, base: definition.apiBase, path: `./manga/detail/${encodeURIComponent(detailKey)}`, headers: { Origin: "https://app.shinigami.asia", Referer: "https://app.shinigami.asia/" }, provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 300_000 });
...
async function getChapters({ mangaId, fetchImpl, timeoutMs, signal }) {
  ...
  for (let page = 1; page <= maxPages; page += 1) {
    const payload = await requestJson({ endpointDefinition, base: definition.apiBase,
...
async function getPages({ chapterId, fetchImpl, timeoutMs, signal }) {
  ...
  const payload = await requestJson({ endpointDefinition, base: definition.apiBase, path: `./chapter/detail/${encodeURIComponent(id)}`, provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 300_000 });

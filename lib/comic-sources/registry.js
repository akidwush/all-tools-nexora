...
const gilakomik = require("./gilakomik");
...
function sourceClassification(definition = {}) {
    ...
    participatesInSearch: definition.participatesInSearch !== false,
    participatesInFallback: definition.participatesInFallback !== false
  };
}

function createComicSourceRegistry(legacy, options = {}) {
  const adapters = [createMangaDexAdapter(legacy), shinigami, voratoon, ainzscans, mangadotnet, gilakomik];

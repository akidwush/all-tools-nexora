...
function mapManga(row = {}) {
  return normalizeManga(definition.id, { id: row.id, title: row.title || row.name, altTitles: row.alt_titles || row.altTitles, coverUrl: row.cover || row.cover_url || row.thumbnail || row.image, description: row.description || row.synopsis, authors: row.authors || (row.author ? [row.author] : []), artists: row.artists || (row.artist ? [row.artist] : []), genres: row.genres, status: row.status, language: row.language || null, url: row.id != null ? `${definition.website}/manga/${encodeURIComponent(row.id)}` : null, metadata: { mediaType: row.type || "Komik" } });
}
async function listPage({ page = 1, limit = 24, fetchImpl, timeoutMs, signal }) {
  const payload = await requestJson({ base: definition.apiBase, path: "./manga", params: { limit: positiveInt(limit, { min: 1, max: 50, fallback: 24 }), page: positiveInt(page, { max: 2000 }) }, provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 120_000 });
  return Array.isArray(payload?.manga_list) ? payload.manga_list : [];
}
...
async function getPages({ chapterId, fetchImpl, timeoutMs, signal }) {
  ...
  return rows.map((row, index) => normalizePage(definition.id, { chapterId: raw, index, url: new URL(row?.url, definition.website).toString(), width: row?.w || row?.width, height: row?.h || row?.height, refererRequired: false }));
}

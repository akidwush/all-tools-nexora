...
const endpointDefinition = Object.freeze({
  id: "comic:mangadotnet", group: "comic", label: "MangaDotNet", category: "comic",
  allowedHosts: ["mangadot.net"], defaultBaseUrl: definition.apiBase,
  defaultTimeoutMs: 10_000, minTimeoutMs: 1_000, maxTimeoutMs: 60_000,
  healthPath: "/api/manga?page=1&limit=1", healthMethod: "GET",
...
async function getChapters({ mangaId, language, fetchImpl, timeoutMs, signal }) {
  ...
  const [chaptersPayload, volumesPayload] = await Promise.all([
    requestJson({ endpointDefinition, base: definition.apiBase, path: `./manga/${encodeURIComponent(manga)}/chapters/list`, provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 120_000 }),
    requestJson({ endpointDefinition, base: definition.apiBase, path: `./manga/${encodeURIComponent(manga)}/volumes`, provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 120_000 }).catch(() => [])
  ]);
...
async function getPages({ chapterId, fetchImpl, timeoutMs, signal }) {
  ...
  const payload = await requestJson({ endpointDefinition, base: definition.apiBase, path, provider: definition.label, fetchImpl, timeoutMs, signal, cacheMs: 300_000 });

...
function normalizeBaseUrl(value, definition) {
  ...
  if (definition.networkSupported === false && !raw) return "";
  let parsed;
...
function defaultConfig(definition) {
  const requestedMode = clean(definition.defaultMode || "active", 24).toLowerCase();
  return {
    providerId: definition.id,
    group: definition.group,
    label: definition.label,
    mode: MODES.has(requestedMode) ? requestedMode : "active",
    ...
    healthPath: definition.networkSupported === false ? "" : normalizeRelativePath(definition.healthPath || definition.defaultPath || "/"),
    fallbackProvider: "",
...
function normalizeCandidate(definition, rawCandidate = {}, options = {}) {
  ...
  return {
    ...
    healthPath: definition.networkSupported === false ? "" : normalizeRelativePath(raw.healthPath == null ? base.healthPath : raw.healthPath, definition.healthPath || definition.defaultPath || "/"),
    fallbackProvider
  };
}
...
async function validateNetworkTarget(definition, effective, options = {}) {
  if (definition.networkSupported === false) {
    throw endpointError("ENDPOINT_NETWORK_UNSUPPORTED", definition.unsupportedReason || "Provider belum mempunyai normal HTTP endpoint yang didukung.", 503);
  }
...
async function testCandidate(definition, rawCandidate, runtime = {}) {
  ...
  // A provider explicitly classified as network/health unsupported has no
  // editable network candidate to validate. Return the truthful health state
  // before normalizeCandidate() so non-editable baseUrl/timeout/healthPath
  // defaults are never mistaken for admin edits.
  if (definition.healthUnsupported === true) {
    return {
      providerId: definition.id,
      status: "unsupported",
      httpStatus: null,
      ...
      resolvedHost: null,
      finalUrl: null,
      error: definition.unsupportedReason || "Provider belum mempunyai health probe yang aman."
    };
  }

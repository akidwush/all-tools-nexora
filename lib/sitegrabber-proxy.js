...
const { validateUrlSyntax } = require("./audit");
...
function validateTargetUrl(raw) {
  try { return validateUrlSyntax(String(raw || "").trim()).toString(); }
  catch (error) {
    throw Object.assign(new Error("URL target harus publik dan menggunakan HTTP/HTTPS yang aman."), {
      status: 400,
      code: error?.code || "INVALID_TARGET_URL"
    });
  }
}

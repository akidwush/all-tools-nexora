...
(function(){
  ...
  window.renderImageVectorizer=function(body){
      ...
      <main class="nvi" aria-label="Nexora Image Vectorizer">
        <section class="nvi-hero">
          <div class="nvi-hero-copy">
            <div class="nvi-kicker"><i class="fa-solid fa-bezier-curve"></i><span>LOCAL VECTOR ENGINE</span><b>VTRACER · HF9.1</b></div>
    ...
    var state={file:null,objectUrl:null,width:0,height:0,preset:'illustration',style:'spline',svg:'',worker:null,requestId:'',busy:false,bootTimer:null,processTimer:null,progressTimer:null,progress:0,destroyed:false};
    ...
    async function run(){
      ...
      try{
        ...
        var workerUrl=new URL('assets/js/workers/vtracer-worker.js?v=6.3.13-hf9.1',document.baseURI).href;state.worker=new Worker(workerUrl,{name:'nexora-vtracer-hf9-1'});
        ...
        state.worker.onerror=function(event){if(event&&typeof event.preventDefault==='function')event.preventDefault();failRun(safeWorkerError(event&&event.message));};
    ...
    function finish(data){
      ...
      var analysis=analyzeSvg(state.svg);var svgBytes=new TextEncoder().encode(state.svg).byteLength;terminateWorker();setProgress(100,'SVG selesai');setBusy(false);vectorHost.innerHTML=state.svg;vector.hidden=false;empty.hidden=true;bgTools.hidden=false;actions.hidden=false;stats.hidden=false;stage.classList.remove('is-empty');stage.classList.remove('view-original','view-split');stage.classList.add('view-vector');root.querySelectorAll('#nviViewTabs button').forEach(function(button){button.classList.toggle('is-active',button.dataset.view==='vector');});vectorLabel.textContent=(data.width||state.width)+' × '+(data.height||state.height)+' · fit 100% · '+(data.downscaled?'optimized':'native');statusBadge.className='is-success';statusBadge.innerHTML='<i class="fa-solid fa-circle-check"></i> VECTOR READY';runButton.querySelector('span').textContent='Vectorize ulang';var ratio=state.file?Math.round((1-svgBytes/state.file.size)*100):0;stats.replaceChildren(metric('fa-solid fa-bezier-curve','Paths',analysis.paths.toLocaleString('id-ID'),'curve & shape'),metric('fa-solid fa-palette','Colors',(analysis.colors||data.colorCount||0).toLocaleString('id-ID'),'quantized palette'),metric('fa-solid fa-file-code','SVG size',formatBytes(svgBytes),ratio>0?ratio+'% lebih kecil':'vector output'),metric('fa-regular fa-clock','Process',(data.durationMs/1000).toFixed(1)+' s','worker thread'));codeText.textContent=state.svg;root.querySelector('#nviCodeSize').textContent=formatBytes(svgBytes);requestAnimationFrame(function(){if(matchMedia('(max-width:720px)').matches)root.querySelector('.nvi-preview-panel').scrollIntoView({behavior:'smooth',block:'start'});});notify('SVG siap diunduh dengan tampilan fit 100%.','is-success');
    }

...
(function(){
  ...
  window.renderPromptGenerator=function(body){
        ...
        <div class="nx-prompt-layout">\
          <form id="nxPromptForm" class="nx-prompt-builder">\
            ...
            <div id="nxPromptDrop" class="nx-prompt-drop" aria-label="Area unggah gambar">\
              <div id="nxPromptEmpty" class="nx-prompt-drop-empty"><i class="fa-solid fa-image"></i><strong>Pilih atau jatuhkan gambar</strong><span>Maksimal 12 MB sebelum optimasi</span><button id="nxPromptChoose" type="button"><i class="fa-solid fa-folder-open"></i> Pilih Gambar</button></div>\
    ...
    function openFilePicker(){if(state.processing)return;fileInput.value='';fileInput.click();}
    ...
    choose.addEventListener('click',function(event){event.stopPropagation();openFilePicker();});drop.addEventListener('click',function(event){if(!event.target.closest('button'))openFilePicker();});fileInput.addEventListener('change',function(){selectFile(fileInput.files&&fileInput.files[0]);});

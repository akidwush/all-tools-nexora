(function () {
  ...
  window.renderHd4Enhancer = function renderHd4Enhancer(body) {
    ...
    body.innerHTML = "" +
      '<main class="nxhd4">' +
        '<section class="nxhd4-hero">' +
          ...
          '<div><span class="nxhd4-kicker">NEXORA IMAGE LAB</span><h2>Nexora Image HD Enhancer V4</h2><p>Tingkatkan detail dan kualitas gambar melalui link gambar langsung.</p></div>' +
        '</section>' +
        ...
        '</section>' +
        '<p class="nxhd4-note"><i class="fa-solid fa-shield-halved"></i> API key diproses aman di server Nexora. URL hasil tidak disimpan.</p>' +
    ...
    form.addEventListener("submit", async function (event) {
      ...
      if (inputUrl.length > MAX_URL_LENGTH || !validUrl(inputUrl)) {
      ...
      sessionState.originalPreview = inputUrl;
      sessionState.error = "";
      setLoading(true);
      renderState();
      ...
      try {
        var query = new URLSearchParams({ url: inputUrl });
        var response = await fetch("/api/tools/hd4?" + query.toString(), {
          method: "GET",
          cache: "no-store",

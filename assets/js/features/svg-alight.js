...
(function(){
  ...
  window.renderSvgAlight=function(body){
    ...
    var state={file:null,svg:'',xml:'',url:null,busy:false,destroyed:false,quality:'lossless',controller:null};
    ...
    async function health(){var c=new AbortController(),t=setTimeout(function(){c.abort();},6500);try{var r=await fetch('/api/svg-alight',{cache:'no-store',signal:c.signal}),d=await r.json();status.className=d.configured?'ok':'warn';status.innerHTML=d.configured?'<i class="fa-solid fa-key"></i> API READY':'<i class="fa-solid fa-triangle-exclamation"></i> KEY?';status.title=d.configured?'API key tersedia; konversi penuh diverifikasi saat file dikirim.':'API key SVG belum tersedia.';}catch(_){status.className='warn';status.textContent='OFFLINE';}finally{clearTimeout(t);}}
    ...
    applyMode('lossless');health();body.__nxCleanup=function(){state.destroyed=true;if(state.controller)state.controller.abort();clean();clearTimeout(toast._t);};
  };
})();

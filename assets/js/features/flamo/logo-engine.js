/* Native adapter for the complete local Flamo Logo Animate XML template set. */
(function(global){
  'use strict';
  var cache=null;
  ...
  function generate(options){
    ...
    return load().then(function(templates){
      ...
      if(opts.addText!==false){
        ...
        text=text.split('<content>Flamo Tools</content>').join('<content>'+esc(content)+'</content>');
        text=text.split('label="Flamo Tools"').join('label="'+esc(content)+'"');
  ...
  global.FlamoLogoAnimateEngine=Object.freeze({ready:true,load:load,generate:generate});
})(window);

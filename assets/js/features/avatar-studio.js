(function(){
  ...
  var API_ORIGIN="https://api.dicebear.com";
  var API_BASE=API_ORIGIN+"/10.x";
  var FALLBACK_STYLES=["lorelei","adventurer","avataaars","bottts","pixel-art","identicon","initials","shapes"];
  var FORMATS=["svg","png","jpg","webp","avif","json"];
  var RASTER_API_MAX=256;
  var STYLE_CACHE_KEY="nxAvatarStudioStylesV10";
  var SAVE_KEY="nxAvatarStudioSavedV1";
  var RECENT_KEY="nxAvatarStudioRecentV1";
  var optionsCache=new Map();
  var definitionCache=new Map();
  var styleListCache=null;
  var activeSchemaController=null;
  ...
  function title(v){return text(v).replace(/[-_]+/g," ").replace(/\b\w/g,function(c){return c.toUpperCase();});}
  function clone(v){return JSON.parse(JSON.stringify(v));}
  function create(tag,cls,content){var n=document.createElement(tag);if(cls)n.className=cls;if(content!=null)n.textContent=String(content);return n;}
  ...
  function retryDelay(response,attempt){var raw=response&&response.headers&&response.headers.get("Retry-After");if(raw){var seconds=Number(raw);if(Number.isFinite(seconds))return Math.min(8000,Math.max(350,seconds*1000));var when=Date.parse(raw);if(Number.isFinite(when))return Math.min(8000,Math.max(350,when-Date.now()));}return Math.min(4000,400*Math.pow(2,attempt));}
  async function fetchJson(url,signal,retries){retries=retries==null?2:retries;var last;for(var i=0;i<=retries;i++){try{var response=await fetch(url,{method:"GET",mode:"cors",credentials:"omit",cache:"default",headers:{Accept:"application/json"},signal:signal});if(response.status===429&&i<retries){await wait(retryDelay(response,i),signal);continue;}if(!response.ok)throw new Error("HTTP "+response.status);return await response.json();}catch(error){if(error&&error.name==="AbortError")throw error;last=error;if(i<retries)await wait(400*Math.pow(2,i),signal);}}throw last||new Error("Request gagal");}

  function randomSeed(){var a=new Uint32Array(3);if(window.crypto&&crypto.getRandomValues)crypto.getRandomValues(a);else{a[0]=Date.now();a[1]=Math.floor(Math.random()*0xffffffff);a[2]=Math.floor(Math.random()*0xffffffff);}return "Nexora-"+Array.from(a).map(function(n){return n.toString(36);}).join("-").slice(0,28);}
  function schedulePreview(){clearTimeout(previewTimer);previewTimer=setTimeout(updatePreview,160);}
  function readStyleCache(){try{var v=JSON.parse(sessionStorage.getItem(STYLE_CACHE_KEY)||"null");if(v&&Array.isArray(v.styles)&&Date.now()-Number(v.at||0)<21600000)return v.styles;}catch(_){}return null;}
  function writeStyleCache(styles){try{sessionStorage.setItem(STYLE_CACHE_KEY,JSON.stringify({at:Date.now(),styles:styles}));}catch(_){}}
  async function discoverStyles(){if(styleListCache&&styleListCache.length)return styleListCache.slice();var cached=readStyleCache();if(cached&&cached.length){state.discovery="session-cache";styleListCache=cached.slice();return styleListCache.slice();}try{var data=await fetchJson(API_BASE+"/",undefined,2);var styles=Array.isArray(data&&data.styles)?data.styles.map(slug).filter(Boolean):[];if(!styles.length)throw new Error("Style list kosong");styleListCache=Array.from(new Set(styles)).sort();state.discovery="dynamic";writeStyleCache(styleListCache);return styleListCache.slice();}catch(error){styleListCache=FALLBACK_STYLES.slice();state.discovery="fallback";return styleListCache.slice();}}
  async function loadOptions(style){style=slug(style);if(optionsCache.has(style))return optionsCache.get(style);if(activeSchemaController)activeSchemaController.abort();var controller=new AbortController();activeSchemaController=controller;try{var data=await fetchJson(API_BASE+"/"+encodeURIComponent(style)+"/options.json",controller.signal,2);var schema=data&&typeof data==="object"&&!Array.isArray(data)?data:{};optionsCache.set(style,schema);return schema;}finally{if(activeSchemaController===controller)activeSchemaController=null;}}
  async function loadDefinition(style){style=slug(style);if(definitionCache.has(style))return definitionCache.get(style);try{var data=await fetchJson(API_BASE+"/"+encodeURIComponent(style)+"/definition.json",undefined,1);definitionCache.set(style,data);return data;}catch(_){definitionCache.set(style,null);return null;}}

  function valuesOf(d){if(!d||typeof d!=="object")return[];var v=d.values||d.enum||d.options||d.choices;if(Array.isArray(v))return v.map(String);if(v&&typeof v==="object")return Object.keys(v);return[];}
  function typeOf(d){var t=text(d&&d.type).toLowerCase();if(t)return t;if(valuesOf(d).length)return"enum";if(d&&("min" in d||"max" in d))return"range";return"string";}
  function defaultOf(d){if(!d)return undefined;if(Object.prototype.hasOwnProperty.call(d,"default"))return d.default;if(Object.prototype.hasOwnProperty.call(d,"defaultValue"))return d.defaultValue;}
  function groupFor(key){var k=key.toLowerCase();if(k==="seed"||/^(base|id|name)/.test(k))return"Identity";if(/background|gradient/.test(k))return"Background";if(/hair/.test(k))return"Hair";if(/eye|brow|glasses/.test(k))return"Eyes";if(/mouth|lip|smile|teeth/.test(k))return"Mouth";if(/accessor|earring|hat|cap|mask/.test(k))return"Accessories";if(/color|skin/.test(k))return"Colors";if(/flip|rotate|scale|size|margin|translate|radius/.test(k))return"Transform";if(/face|nose|facial|beard/.test(k))return"Face";return"Advanced";}
  var GROUP_ORDER=["Identity","Face","Hair","Eyes","Mouth","Accessories","Colors","Background","Transform","Advanced"];
  function appendParam(params,key,value){if(value==null||value==="")return;if(Array.isArray(value)){value.forEach(function(v){if(v!==""&&v!=null)params.append(key,String(v));});}else params.set(key,String(value));}
  function buildUrl(format){format=FORMATS.indexOf(format)>=0?format:"svg";var url=new URL(API_BASE+"/"+encodeURIComponent(state.style)+"/"+format);var params=new URLSearchParams();params.set("seed",state.seed||"Nexora");Object.keys(state.options).sort().forEach(function(key){appendParam(params,key,state.options[key]);});if(format!=="svg"&&format!=="json"&&Number(state.rasterSize)<=RASTER_API_MAX)params.set("size",String(Math.max(1,Math.min(RASTER_API_MAX,Number(state.rasterSize)||256))));url.search=params.toString();return url;}
...
`;
  ...
  function setOption(key,value){var had=Object.prototype.hasOwnProperty.call(state.options,key),previous=had?clone(state.options[key]):undefined;state.lastMutation={key:key,had:had,previous:previous};if(value==null||value===""||(Array.isArray(value)&&!value.length))delete state.options[key];else state.options[key]=value;schedulePreview();}
  ...
  async function copy(value,message){await navigator.clipboard.writeText(value);status(message||"Copied","ok");}
  async function fetchBlob(url,accept){var response=await fetch(url,{mode:"cors",credentials:"omit",headers:{Accept:accept||"*/*"}});if(!response.ok)throw new Error("HTTP "+response.status);return await response.blob();}
  function saveBlob(blob,name){var u=URL.createObjectURL(blob),a=document.createElement("a");a.href=u;a.download=name;document.body.append(a);a.click();a.remove();setTimeout(function(){URL.revokeObjectURL(u);},1000);}
  async function downloadSvg(){setBusy(true,"Fetching SVG…");try{saveBlob(await fetchBlob(buildUrl("svg"),"image/svg+xml"),"avatar-nexora.svg");status("SVG downloaded","ok");}catch(e){status("Download SVG gagal: "+e.message,"error");}finally{setBusy(false);}}
  async function clientPng(size){var blob=await fetchBlob(buildUrl("svg"),"image/svg+xml"),u=URL.createObjectURL(blob);try{var img=new Image();img.crossOrigin="anonymous";await new Promise(function(resolve,reject){img.onload=resolve;img.onerror=reject;img.src=u;});var canvas=document.createElement("canvas");canvas.width=size;canvas.height=size;canvas.getContext("2d").drawImage(img,0,0,size,size);return await new Promise(function(resolve){canvas.toBlob(resolve,"image/png",1);});}finally{URL.revokeObjectURL(u);}}
  async function downloadPng(){var size=Number(state.rasterSize)||256;setBusy(true,size>RASTER_API_MAX?"Rendering PNG locally…":"Fetching PNG…");try{var blob=size>RASTER_API_MAX?await clientPng(size):await fetchBlob(buildUrl("png"),"image/png");if(!blob)throw new Error("PNG export gagal");saveBlob(blob,"avatar-nexora-"+size+".png");status(size>RASTER_API_MAX?"PNG rendered locally from SVG":"PNG downloaded from DiceBear","ok");}catch(e){status("Download PNG gagal: "+e.message,"error");}finally{setBusy(false);}}
  async function downloadSelected(){if(state.format==="svg")return downloadSvg();if(state.format==="png")return downloadPng();setBusy(true,"Fetching "+state.format.toUpperCase()+"…");try{var accept=state.format==="json"?"application/json":"image/"+(state.format==="jpg"?"jpeg":state.format);saveBlob(await fetchBlob(buildUrl(state.format),accept),"avatar-nexora."+state.format);status(state.format.toUpperCase()+" downloaded","ok");}catch(e){status("Export gagal: "+e.message,"error");}finally{setBusy(false);}}

  function readSaved(){try{var r=JSON.parse(localStorage.getItem(SAVE_KEY)||"[]");return Array.isArray(r)?r:[];}catch(_){return[];}}
  function writeSaved(rows){try{localStorage.setItem(SAVE_KEY,JSON.stringify(rows.slice(0,20)));}catch(_){}renderSaved();}
  function renderSaved(){var host=q("[data-as-saved]");if(!host)return;host.replaceChildren();var rows=readSaved();if(!rows.length){host.append(create("div","as-empty","Belum ada config tersimpan."));return;}rows.slice(0,8).forEach(function(item){var row=create("div","as-saved-row"),load=btn(item.name,"as-saved-load"),del=btn("×","as-btn as-btn-soft");load.addEventListener("click",function(){importConfigObject(item.config);});del.addEventListener("click",function(){writeSaved(readSaved().filter(function(x){return x.id!==item.id;}));});row.append(load,del);host.append(row);});}
  function savePreset(){var name=prompt("Nama preset/avatar:","Avatar "+title(state.style));if(!name)return;var rows=readSaved();rows.unshift({id:Date.now().toString(36),name:name.slice(0,50),config:currentConfig(),savedAt:new Date().toISOString()});writeSaved(rows);status("Preset disimpan lokal","ok");}
  function saveRecent(){try{var rows=JSON.parse(sessionStorage.getItem(RECENT_KEY)||"[]");if(!Array.isArray(rows))rows=[];var c=currentConfig(),k=JSON.stringify(c);rows=rows.filter(function(x){return JSON.stringify(x)!==k;});rows.unshift(c);sessionStorage.setItem(RECENT_KEY,JSON.stringify(rows.slice(0,10)));}catch(_){}}
  function encodeConfig(config){var bytes=new TextEncoder().encode(JSON.stringify(config)),binary="";bytes.forEach(function(b){binary+=String.fromCharCode(b);});return btoa(binary).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/g,"");}
  ...
  window.NexoraAvatarStudio=Object.freeze({API_BASE:API_BASE,FALLBACK_STYLES:FALLBACK_STYLES.slice(),FORMATS:FORMATS.slice(),buildUrl:function(format){return buildUrl(format).toString();},currentConfig:currentConfig,discoverStyles:discoverStyles,loadOptions:loadOptions,mount:mount});
})();

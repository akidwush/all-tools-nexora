...
function generateXml(layers,o,doc,fontName){const totalMs=Math.max(1,Math.round(o.duration*1000)),shapes=[...layers].reverse().map((layer,i)=>{const contours=layer.contours.map(c=>`      <contour${c.exclude?' exclude="true"':''} d="${pathAttr(c.d)}"/>`).join('\n'),fill=o.vectorStyle==='strokeOnly'?'':`\n    <fillColor value="${argb(o.fill)}"/>`,stroke=o.vectorStyle==='fillOnly'?'':`\n    <path-stroke direction="centered" join="round" width="${fmt(o.strokeWidth)}"><color value="${argb(o.stroke)}"/></path-stroke>`;return `  <shape id="${i+1}" label="${xmlEscape(layer.name)}" startTime="0" endTime="${totalMs}" fillType="${o.vectorStyle==='strokeOnly'?'none':'color'}"><transform><location value="${fmt(layer.x)},${fmt(layer.y)},0.000000"/><pivot value="0.000000,0.000000"/><scale value="1.000000,1.000000"/></transform>${fill}<path>\n${contours}\n    </path>${stroke}\n  </shape>`;}).join('\n');return `<?xml version='1.0' encoding='UTF-8' ?>\n<!-- Generated by Flamo Text to Vector · local OpenType -->\n<scene title="${xmlEscape(o.filename||'text-to-vector')}" width="${doc.width}" height="${doc.height}" exportWidth="${doc.width}" exportHeight="${doc.height}" bgcolor="#FF000000" totalTime="${totalMs}" fps="${o.fps}" modifiedTime="${Date.now()}" amver="859" ffver="107" am="com.alightcreative.motion/6.2.53" amplatform="ios" precompose="dynamicResolution" retime="freeze">\n${shapes}\n</scene>`;}
